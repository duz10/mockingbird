...
use kg_validation::synonyms::SynonymMap;
...
FLAGS:
  ...
  --synonym-map <path>    Optional synonym map JSON (default: judge-calibration/synonym-map.json)
  --no-synonym-map        Force-disable synonym map even when default file exists
...
fn real_main() -> anyhow::Result<()> {
    ...
    // Wave 0.5.3 iter 3 (`mb-e10v`): the synonym map participates
    // IN-BAND at validate-time on closed-vocab schemas. Default is
    // the canonical `judge-calibration/synonym-map.json` so the
    // happy path needs no flags. `--no-synonym-map` is the escape
    // hatch for reproducing iter-1/iter-2 pre-fix behaviour.
    let mut synonym_map_path: Option<PathBuf> = None;
    let mut no_synonym_map = false;
    ...
    while i < args.len() {
        ...
        match arg {
            ...
            "--synonym-map" => {
                synonym_map_path = Some(PathBuf::from(take_value(&args, &mut i, "--synonym-map")?));
            }
            "--no-synonym-map" => {
                no_synonym_map = true;
            }
    ...
    // Resolve the synonym map. Precedence:
    //   1. `--no-synonym-map` → None (force-disabled).
    //   2. `--synonym-map <path>` → load that path (fail loudly).
    //   3. default path `judge-calibration/synonym-map.json` if it
    //      exists → load it. Missing file → None (open-vocab schemas
    //      don't need it; legacy harness behaviour preserved).
    let synonym_map: Option<SynonymMap> = if no_synonym_map {
        None
    } else {
        let resolved_path = synonym_map_path
            .unwrap_or_else(|| PathBuf::from("judge-calibration").join("synonym-map.json"));
        if resolved_path.exists() {
            let map = SynonymMap::load(&resolved_path).map_err(|e| {
                anyhow::anyhow!(
                    "failed to load synonym map at {}: {e}",
                    resolved_path.display()
                )
            })?;
            println!(
                "synonym-map : {} ({} entries, v{})",
                resolved_path.display(),
                map.variant_to_canonical.len(),
                map.version,
            );
            Some(map)
        } else {
            println!(
                "synonym-map : <none> (no file at {})",
                resolved_path.display()
            );
            None
        }
    ...
    let summary = run_corpus(&client, &schema, synonym_map.as_ref(), &config);
