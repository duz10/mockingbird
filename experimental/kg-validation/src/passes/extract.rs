...
//!
//! The date hard-gate (spec §8.4) lives both in the prompt (HARD
//! GATE instructions + an explicit "soon → null" example) and in
//! validation here: a non-null `due_iso` must be parseable as
//! `YYYY-MM-DD` or the pass reports a `Validation` error so the
//! scorer can flag the dictation. We do NOT silently re-coerce a
//! malformed date to `null`, because that would mask an LLM that's
//! shouting "I want to invent a date" — Wave 3 needs to see the
//! shouting.
...
pub fn extract<D: OllamaDispatcher>(
    ...
    // Validate: due_iso, when Some, must be YYYY-MM-DD.
...
/// after that, giving the model an explicit anchor when the
/// speaker says "Monday" vs "next Monday."
///
