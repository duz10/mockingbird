...
Phase MC is the standalone "record a long-form session, get a clean transcript, optionally run an LLM pass" feature. It targets parity with WisprFlow's meeting mode but local-only. Activation is **double-tap** on a configurable key (default Pause/Break; F23/F24 fallback per ADR 0019 conflict-probe). On stop, the audio is fed through a **chunked Whisper inference path** (the long-form fix the standing P1 `mb-2bi` has been queued for — Phase MC ships it), then a **deterministic formatter** (filler removal + paragraph breaks from segment timestamps + capitalization), then saved as the canonical transcript. No LLM runs in the critical path. The user can optionally trigger a secondary LLM pass against the canonical transcript and export the result as a file — not persisted.

## Overview

Phase MC adds a meeting-recording feature that lives alongside dictation. Trigger: double-tap Pause/Break in any focused app → an overlay surfaces three audio-source choices (Microphone / System / Both). Press Start → the audio capture pipeline runs until the user presses Stop (or the configured ceiling fires; default 4 hours, hard ceiling 6 hours). On stop:
...
/// double-tap timeout drift, twin-stream init, loopback probe).
...
│   ├── activation.rs           # Double-tap listener (second WH_KEYBOARD_LL)
...
```
IDLE
  └─ on key_down(meeting_key)             → ARMED { first_press_ts: now }

ARMED { first_press_ts }
  ├─ on key_up                            → WAITING_SECOND { since: now }
  └─ on tick (now - first_press_ts > 800) → IDLE  (single tap consumed; pass through)

WAITING_SECOND { since }
  ├─ on key_down(meeting_key)
  │     and (now - since) ∈ [80, 400]     → emit MeetingToggle { source = LAST_CHOSEN }
  │                                          → IDLE
  ├─ on key_down(meeting_key)
  │     and (now - since) > 400            → IDLE  (too slow; consumed as a fresh first press)
  └─ on tick (now - since > 800)           → IDLE  (no second tap; single tap forgotten)

Edge cases (verbatim test inputs in formatter.rs siblings):
- Double-tap fires DURING an in-progress meeting → emits MeetingToggle, which the
  runtime interprets as Stop. (Activation is a toggle.)
- Double-tap fires DURING a dictation hold → ignored by activation (the dictation
  hook owns its own key; meeting hook only sees the meeting key). If a user
  configures the same VK for both, the conflict probe at startup rejects it.
- Pause-meeting tray toggle (Phase MC tray-menu addition) → activation events no-op.
- Triple-tap → treated as double-tap + single-tap-pending. The third press starts
  a new ARMED state. (Behaviour is deterministic; we just don't act on the third.)
```

The state machine takes `ActivationEvent` inputs (`KeyDown { ts }`, `KeyUp { ts }`, `Tick { ts }`, `PauseToggle { paused }`) and emits `ActivationAction` outputs (`MeetingToggle { source }`, `Noop`). Test density target: ≥20 tests covering all edges (single-tap-too-slow, double-tap-too-fast, double-tap-too-slow, triple-tap, hold-then-tap, etc.).
...
```rust
MeetingHotkey,                  // string VK name; default "VK_PAUSE", fallback "VK_F23"
MeetingDoubleTapWindowMs,       // u32; default 400, clamp [150, 800]
...
| bd-task title (prefix `Phase MC:`) | priority | files |
|-----------------------------------|----------|-------|
| `meetings/activation.rs` — pure double-tap state machine per Section MC.1 with ≥20 unit tests | P0 | `src-tauri/src/meetings/activation.rs` |
...
| Conflict probe: if `MeetingHotkey` equals the dictation `hotkey.binding`, reject at startup with tray toast + log; fallback ladder Pause → F23 → F24 → user-pick (UI-pick deferred to Wave 5) | P1 | `src-tauri/src/meetings/activation.rs` + extend `src-tauri/src/hotkey/probe.rs` interface |
...
1. **Functional:** Double-tap Pause/Break with no app focused → overlay appears, source pickable, Start/Stop works, transcript appears in the new Meetings page on stop. Verified manually by Dustin in:
...
| 4 | **Double-tap window mis-tuned** (too tight → user can't trigger; too loose → triggers on regular typing) | **Medium** | Default 400 ms is the WisprFlow / iOS-keyboard typical range. Setting is exposed in Settings → Meeting → Activation. Conflict probe + tray toast on startup if the user's chosen key collides with the dictation hotkey. |
...
| 6 | **User triggers double-tap during a dictation hold** | **Low** | Different VK — meeting hook only sees its own key, dictation hook only sees its own key. Conflict probe at startup rejects the same-VK configuration. If a user somehow forces it (manual settings edit), the meeting hook's `Noop` branch handles the case (it ignores its activation if the dictation state is `Recording`, queried via a non-blocking peek into the dictation state). |
