...
| 2026-06-04 | `[KG Phase 1D Wave 1D.6 SEAL / mb-q2p1 / ADR 0052]` | Three judge-authoring shape findings from the Phase 1D seal: (1) **per-cell fresh DB for matrix probes makes failure blame unambiguous** — J1's 3×2 corpus (3 `capture_kind`s × 2 toggle states) uses one tempfile DB per cell so a regression message says exactly *which* cell mis-fired, instead of a corpus-total "observed 3, expected 2". Cost is hundreds of ms of extra `NamedTempFile::new()` + `apply_all()`; payoff is the diagnostic shape Phase MC's `mc-formatter-deterministic` already uses. (2) **"Extend judge X" charter text often collapses to "author a sibling judge to X" once you draw the diff-vs-runtime distinction** — J2's charter said "extend Phase MC's `mc-dictation-untouched`", but Phase MC's judge is a diff judge over a commit range, and Wave 1D.1 already legitimately added the source gate to `dictation.rs` (which would trip the literal diff assertion). The authentic shape is a Phase 1D **runtime twin** of MC's judge — same invariant ("standard dictations produce zero `kg_*` writes"), different assertion mechanism (runtime probe vs commit-range diff), robust to authorized file-level surgery. When a charter's "extend X" framing is incompatible with the new state of the world, the diff-vs-runtime distinction is the most common axis along which the framing collapses to "sibling of X". (3) **A "no new code shipped" judge is a legitimate shape** — J3's deliverable was a doc enumerating the consolidated Playwright invariant set (already authored across 1D.2/1D.4/1D.5) as fully satisfied, with criteria + last-run evidence pointing back to each component wave. The cargo-cult instinct ("every judge needs a new spec file") would have added redundant assertions to an already-complete spec; the doc IS the deliverable. Body, narrow blast radius (judge-authoring process) |
| 2026-06-04 | `[KG Phase 1D Wave 1D.4 / mb-6hm2 / ADR 0052]` | Subtractive relocation wave done as cheaply as possible: rewrite-with-overwrite for the gutted files (Dictations.tsx / DictationsList.tsx / SettingsKgTab.tsx), `git mv` for spec renames, single targeted `replace_in_file` for DictationsDetailPane's prop removal, brand-new file for the destination band. Two findings worth keeping: (1) **dead-state sparse columns are an anti-pattern; remove them from the wrong surface entirely instead of "documenting the sparse-population semantic"** — `sessions.category` is populated ONLY by the KG filing worker on `capture_kind='kg-note'` rows, so on the Dictations page (where `capture_kind='dictation'` is 100% of rows) the category badge would render NULL forever. The wave's (a)/(b) decision call between "remove the badge" vs "keep it with semantic docs" only has one defensible answer once you draw the actual population distribution. Generalization: whenever a UI field is structurally NULL for the entire population of a surface, that field doesn't belong on that surface — moving it to where it's populated is one-and-only. (2) **The graph-off invariant judge's J3 tightening ("Dictations page fires ZERO `kg_*` regardless of toggle state") is the kind of assertion that becomes trivially true via subtraction, not via guards** — the prior shape had `if (toggleOn) fireKgIpcs()` plumbing on Dictations that needed runtime predicate inspection to verify; the post-relocation shape has NO `kg_*` import anywhere in the page module, so the assertion is structurally true at the bundle level, not just at the runtime level. The judge Playwright spec is the regression net; the actual invariant is enforced by the import graph. Pattern: when you can turn a runtime invariant into a structural invariant via dead-code elimination, prefer that over a runtime guard. Narrow blast radius (UI organization), body not PINNED |
| 2026-06-04 | `[KG Phase 1D Wave 1D.3 / mb-0gt6 / ADR 0052]` | Orphan-recovery pattern: when a kickoff warns of API-error-truncated prior dispatches, `git diff --stat <last-seal>..` is the FIRST tool call after `git status`. Wave 1D.3 inventoried 11 modified files + 1 untracked (421-LoC `kg::ingest_text` module + tests + IPC plumbing) before writing a single new line — saved ~500 LoC of solid orphan work from being discarded + re-derived. Plus two narrower findings: (a) the phase doc's anticipated `run_from_text` refactor seam didn't actually exist — `run_pipeline` already takes `&str`, audio and text paths naturally converge at the `transcripts.text` row, ~100 LoC of unneeded plumbing skipped. (b) clippy's `doc_lazy_continuation` treats `+ word` / `- word` / `* word` at the start of a wrapped doc-comment line as a malformed list marker; rephrase to avoid those tokens at line starts ("Read AND cleared at..." instead of "Read\n/// + cleared at..."). Body, narrow blast radius |
| 2026-05-30 | `[KG Phase 1C Wave 1C.4 / mb-sx6p / ADR 0051]` | Concept-modal wave: 2 IPCs + 2 store helpers + `ConceptModal.tsx` (332 LoC) + chip-strip wired click-to-open. All four cargo gates green, UI gates green, KG Playwright specs 3/3 (kg-concept-modal new, kg-dictations-retrieval + kg-failed-filings preserved). Three findings: (1) **when a previously-inert element gains an action, its aria-label MUST change** — chips went from `<span aria-label="Entity Mom">` to `<button aria-label="Open concept page for entity Mom">`, which is correct a11y but regresses every prior spec that locates by the old label; grep `getByLabel\(<entity-name>` at wave-plan time to size the test-debt surface area. (2) **clippy `doc_lazy_continuation` reads `///` continuation lines starting with `+`/`*`/`-`/digit as a malformed list** — reflow into a parenthetical sentence or indent two spaces if you actually want a list. (3) **end-of-iteration Playwright sweeps surface pre-existing failures unrelated to the wave** — `mb-26aw` filed for 4 smoke.spec.ts failures + 1 kg-settings-tab unscoped `getByRole("status")` strict-mode violation that pre-date 1C.4; right move is `git log -- <spec>` to bisect blame, file a bead, carry on. Body, narrow blast radius |
| 2026-05-30 | `[KG Phase 1C Wave 1C.1 / mb-ucmx / ADR 0051]` | First KG-epic wave to touch `ui/**` (Settings tab + activation toggle + boot-vs-poll worker promotion). Five commits, all gates green first try, parity 32/32 both modes, Playwright single-judge sweep green. Three findings: (1) `capabilities/default.json` is wildcard (`core:default`) on this project — the ADR 0035 / `mb-z5y` regression was about a missing `"windows":` array entry (platform-targeting), NOT per-command allowlist; new `#[tauri::command]` functions are exposed via `tauri::generate_handler!` registration alone, no capabilities edit needed. (2) qa-kitten's invocation environment in this Code Puppy install lacks fs-write — route NEW `.spec.ts` authoring to code-puppy; reserve qa-kitten for *running* sweeps and *judging* screenshots. (3) Boot-vs-poll promotion via per-tick settings read costs ~1 SQL per IDLE_SLEEP and the math is fine; pattern (spawn unconditional + gate inside drain loop + fail-closed on poll error) generalizes to any runtime-toggleable worker. Body, narrow blast radius |
| 2026-06-03 | `[KG Phase 1B Chunk 5 SEAL / mb-k17a / ADR 0050]` | Phase 1B sealed first-try (0 retries, ~25 min). Two probe-shape findings for future phases to inherit: (1) one-shot trigger-fire gate decouples from per-fixture loop — schema/trigger-layer invariants don't need to multiply by fixture count; (2) positive-control flip turns a vacuous-pass into a structural assertion — every "feature-off must be a no-op" probe should include a feature-on positive control to prove the helper IS structurally able to write. Both patterns are ~20-line conventions, no library helper. Bonus: promoting `tempfile` from `[dev-deps]` to `[deps]` is right when the project's `#[test]` runner is broken (P2) and the probes are first-class binaries. Body, narrow-blast-radius |
| 2026-06-02 | `[KG Phase 1B Chunk 4 / mb-ryq4 / ADR 0050]` | Chunk 3 escaped its cargo gate with a `fmt --check` regression in `kg/pipeline.rs` (use-list out of alphabetical order) because `fmt --check` was apparently not run separately from `clippy`. The wrapper script doesn't chain the four gate calls; each is independent. Sub-agent dispatches that report "cargo gate green" without itemizing each of `check / fmt --check / clippy --release -- -D warnings / test --release --no-run` have a hidden failure mode. Generalization: end-of-iteration reports must list each of the four with its own ✓ or ✗. Sweep-up rule: stray fmt regressions detected mid-iteration get fixed in the active commit + noted, not stashed or escalated. Narrow blast radius (process discipline), body not PINNED |
| 2026-06-02 | `[KG Phase 1B Chunk 3 / mb-eke8 / ADR 0050]` | Production `run_pipeline` was 4-pass-only despite ADR 0049 §6 binding 5 passes — surfaced when Chunk 3 went to wire `extract_entities` consumption in the filing worker and discovered nobody PRODUCED it. The Wave 0.5.4 entity probe ran `extract_entities` standalone and so does the parity probe (`parity.rs` ~L410), so coverage gap was invisible. Without Chunk 3 wiring it as the 5th pass per ADR 0049 §6, Chunk 2's `kg_entity_mentions` table would be dead-on-arrival. Empirically confirmed Chunk 2's additive-field-invisibility hypothesis (extended `PipelineResult` with `segment_entities`; `kg_parity` stayed 32/32 GREEN). MockOllama needle ordering: extract_entities prompt shares the `\n\nSEGMENT:\n{seg}\n` suffix with classify; disambiguation requires a body-substring needle (`"You extract specific named or concrete"`) registered BEFORE the SEGMENT-keyed classify needle in the rule chain. Future-action: add a `kg-pipeline-passes-all-runtime-reachable` probe to ADR 0050 Chunk 5 acceptance so this gap class can't recur. Body, broad-implication |
| 2026-06-01 | `[KG Phase 1B Chunk 2 / mb-geds / ADR 0050]` | `PipelineResult` is structurally extensible at zero parity-risk: `parity.rs::pipeline_result_to_value` does NOT use `serde_json::to_value(&result)`, it manually builds `json!({ entries, per_pass_errors, new_tag_requests })` from three field accesses. `PipelineResult` has no `#[derive(Serialize)]` either. Adding fields is undetectable to the parity probe by construction; the wave brief's `#[serde(skip)]` framing presumed serde-driven serialization that isn't actually in the code path. When evaluating struct-extension parity safety, check whether the serialization path is serde-derived or hand-built before reaching for `#[serde(skip)]`. (Chunk 2 still picked store-layer-only `SegmentOutput` for YAGNI — empty `PipelineResult.segment_outputs` would be dead code — but Chunk 3 can wire a `PipelineResult` field freely if it wants.) Body, narrow blast radius |
| 2026-05-31 | `[KG Phase 1A Chunk 3 SEAL / mb-2mc9 / mb-cskk]` | Parity-gate authoring caught TWO independent fixture-vs-reality bugs that only surfaced on the corpus's one parse-failure dictation (persona-06-case-05). Bug A (probe): originally asserted `segment_failures` equality, but that field is sandbox-run provenance that's unreproducible from aggregate canned responses — fixed by dropping it from the comparator per `parity/README.md` §3 option 1. Bug B (fixture): `aggregate_fixture.py` hardcoded `pipeline_result.per_pass_errors: []` for all 32 fixtures (which matched reality 31/32 by luck) AND read only `parsed` from sandbox classify/extract artifacts, throwing away `raw_model_output` for the parse-failure case — meaning the canned responses sidecar carried `null` where the sandbox actually had a model response that production needed to reproduce. Fix: `_canned_response_for` falls back to `raw_model_output` when `parsed` is null; `_collect_per_pass_errors` walks every per-pass artifact's `error` field. Production's `PassError::Parse` Display format matches the sandbox's stored error template byte-for-byte, so the round-trip is honest. Pattern lesson: a fixture-aggregator that elides ANY error/raw-output provenance from the source artifacts will pass parity on the happy-path subset and fail mysteriously on the one weird case; capture every observable field even if you think the run was clean. Narrow blast radius (parity-gate authoring), body not PINNED |
| 2026-05-31 | `[KG Phase 1A Chunk 2 / mb-2mc9 / Wave 2 graduation]` | `thiserror` reserves the field name `source` (and `from`) for chainable errors. Picking `source: String` for a "where did this come from" descriptive payload errors as a cryptic trait-bound failure on `String`, not as a field-name collision. Workaround: rename to `loaded_from` / `provenance` / `origin`. Pattern: treat `source` and `from` as thiserror-owned identifiers, use them only when the field implements `std::error::Error`. Body, not PINNED — narrow blast radius (only bites when authoring a new thiserror enum with that exact field-name overlap) |
| 2026-05-31 | `[KG Phase 1A Chunk 1 / mb-2mc9 / mb-8thh]` | PowerShell 5.1 `ConvertTo-Json` wraps `Object[]` values nested in `[ordered]@{}` as `{value, Count}` instead of bare arrays. Bit broke the initial parity-fixture aggregator silently (32 dictations of structurally-correct-but-wrong-shape JSON). Rewrote in Python 3 (`json.dump` is shape-honest, deterministic across re-runs). Pattern lesson: for *assembling* JSON from heterogeneous on-disk JSON inputs, reach for Python not PowerShell. PowerShell is fine for cargo / path / file enumeration / raw-text string surgery; not safe for `ConvertFrom-Json | mutate-in-ordered-hashtable | ConvertTo-Json` round-trips when any value is a nested array. Narrow blast radius (parity fixtures, snapshot tests), body not PINNED |
...
## 2026-06-05 [KG Phase 1E Wave 1E.4 / mb-i14b / ADR 0053 §D7] History archive (per-session JSON sidecar + audio move) shipped; two non-obvious findings: golden JSON fixtures need explicit LF normalization on Windows, and `serde_json::to_string_pretty` is LF-deterministic by contract (worth leaning on)

- **Context:** Wave 1E.4 adds `vault::history` -- a phase-4 (post-seal, post-mark-done) step that writes a canonical-JSON sidecar at `<vault>/Knowledge Graph/History/<YYYY-MM>/<session-uuid>.json` plus moves the source audio recording into the same month-bucket. Same non-fatal-to-queue philosophy as Wave 1E.3's vault projection: if archiving fails, the entry + queue are already sealed; `reconcile_history` recovers on demand.

- **Finding 1 -- Golden JSON fixtures created via the agent's file-creation tool land as CRLF on Windows; they fail byte-identity tests where the actual output is LF-only.** Three goldens fixed today (kg_note_with_audio, kg_note_text, kg_note_sparse) failed first-run with expected.len() = actual.len() + 12 across all three -- one CR per line of a 12-line file. Visible content was byte-for-byte identical in the panic message. `.gitattributes` already specifies `*.json text eol=lf`, so they'd normalize to LF on commit, but the local working tree still had CRLF until I rewrote them via `[System.IO.File]::WriteAllText` with `[System.Text.UTF8Encoding]::new($false)` and an explicit `"\`r\`n" -> "\`n"` replace. **Generalization:** for any test that pins canonical bytes against a fixture file on Windows, either (a) rewrite the fixture via raw bytes after creation, (b) regenerate it via `MOCKINGBIRD_UPDATE_GOLDENS=1` once and let the writer use the same encoder as runtime, or (c) make the comparison line-ending-insensitive. The cheapest in practice is (b) once the test harness is wired up.

- **Finding 2 -- `serde_json::to_string_pretty` emits LF newlines + 2-space indentation unconditionally on every platform.** Confirmed by 35/35 throwaway tests passing after the goldens were LF-normalized. The docs don't make this explicit; it falls out of `PrettyFormatter`'s default constructor using `b"\n"` directly. **Lesson:** safe to lean on for canonical JSON contracts (history sidecars, future schema-snapshot artefacts). If the contract ever requires CRLF, you'd need a custom formatter -- but for our "LF only on every platform" rule this is free correctness.

- **Action:** Future canonical-bytes contracts (e.g. mb-43xw IPC reconcile output if it ever surfaces a JSON artefact) can mirror this module's `serialize_<thing>` pure helper + golden test pattern. Filed mb-* (separate bead) noting that `kg::worker::process_one` is now 1261 lines and worth splitting in a future refactor -- but the file's still cohesive (one process_one with linear stages), so the split is a P3 cleanup, not a structural fix. The history-archive module itself is 586 lines (under cap) and its 682-line sibling test file follows the established `markdown_serializer_tests.rs` precedent (734 lines).

---

## 2026-06-04 [KG Phase 1E Wave 1E.3 / mb-k2pk / ADR 0053] Two-phase commit ordering implemented; three non-obvious findings worth keeping (vocab drift between KG schema and vault serializer; queue-done MUST decouple from vault-projection retry budget; clippy::unnecessary_lazy_evaluations fires on `unwrap_or_else(|| CONST::FIELD)`)

- **Context:** Wave 1E.3 implements the two-phase commit per ADR 0053 §D4 -- migration 026 adds three vault-linkage columns (entry_id / vault_path / vault_file_hash), and the worker grew a step between `apply_filed_outcome` and `mark_done` that builds an in-memory `KgEntry`, records the SHA-256 hash, writes the file atomically, then seals the row. The kickoff specified the ordering but left three implementation choices unspoken; each one mattered.

- **Finding 1 -- Vault and KG `EntryType` vocabularies have drifted.** `kg::schema::EntryType` ships `{Task, Research, Idea, Note, Reference}` (Wave 1A vocabulary, ADR 0049 §6) and `vault::markdown_serializer::EntryType` ships `{Note, Task, Idea, Question, Decision}` (Wave 1E.2 vocabulary). Three variants overlap; two each side disagree. The 1E.3 mapping lossy-collapses `Research` and `Reference` to `Note` for the vault projection -- durable, but it means the markdown frontmatter loses the distinction that `kg_entries.entry_type` keeps. Filed `mb-il83` (P3) to reconcile via a follow-up ADR (probably: extend vault `EntryType` to be a superset, then back-fill the markdown frontmatter from `kg_entries.entry_type` directly). **Lesson:** when two modules ship enum vocabularies in different waves of the same epic, the second wave's author should diff against the first's enum before defining its own. Easy fix; we didn't catch it because Wave 1E.2 (vault serializer) and Wave 1A (KG schema) were written in different parts of the tree.

- **Finding 2 -- The vault projection's retry budget MUST decouple from the LLM-filing queue's retry budget.** The kg_filing_queue uses a 3-attempt fail-then-park policy; if you naively make the vault projection step "part of the filing", any transient file-write failure burns one of those attempts AND leaves the kg_* rows orphaned (because `apply_filed_outcome` is already past). The actual implementation makes the vault projection non-fatal to the queue: if the file write fails after the kg_* rows landed, the worker logs a warning and STILL marks the queue done; the row sits in the "hash set, paths NULL" reconcile signature, and `vault::writer::reconcile_vault` (or future 1E.5 watcher events) recover it. **Generalization:** when a single worker step has two downstream consequences with very different retry profiles (LLM = bounded retries, file I/O = effectively infinite via reconcile), conflating them into one fail/succeed boundary will eventually trip on a transient I/O glitch and lose data. Split the seal txns.

- **Finding 3 -- `clippy::unnecessary_lazy_evaluations` is more aggressive than expected; `unwrap_or_else(|| Type::CONST)` trips it.** I wrote `parse_iso_to_utc_opt(iso).unwrap_or_else(|| chrono::DateTime::<chrono::Utc>::UNIX_EPOCH)` because `UNIX_EPOCH` looks like it might be a function call. It's a const associated item -- clippy correctly insists on `unwrap_or(chrono::DateTime::<chrono::Utc>::UNIX_EPOCH)`. **Lesson:** if the fallback value is a const / literal / cheap field access, always reach for `unwrap_or` first; reserve `unwrap_or_else` for things that involve real work (allocation, syscall, computation). The closure version is strictly more expensive (allocates a closure even if optimized away later) and clippy will catch you on every release-mode pass.

- **Action:** Open ADR follow-up for the `EntryType` vocab reconciliation (mb-il83). For Phase 1E.5 (reverse-watcher), wire the hash-match short-circuit against `sessions.vault_file_hash` -- the substrate is now in place; the watcher just has to recompute SHA-256 on event and compare. The reconcile_vault function is the on-demand recovery surface; mb-43xw will wire it to an IPC, mb-srvh will add a timer-driven nightly sweep.

---

## 2026-06-04 [KG Phase 1E Wave 1E.0 / mb-nuba / ADR 0053] Charter design surfaced three non-obvious calls: hash-based loop-prevention (NOT mtime), DB-first/file-second two-phase commit ordering, and `__<id8>` durable-identity suffix on filenames

- **Context:** Wave 1E.0 is the Phase 1E charter (ADR 0053 + phase doc + bead epic), no implementation work. The charter has to make load-bearing design calls now so the downstream wave dispatches can be one-liners. Three of those calls were non-obvious enough to be worth pinning before they get re-discovered the hard way at 1E.5.

- **Finding 1 — Reverse-watcher loop-prevention MUST key on file content hash, not mtime.** The naive shape is "after writing, record `file_mtime`; on a watcher event, compare event mtime to recorded mtime; if equal → no-op (our own write echoing back)." This breaks in two production-realistic ways. (a) **Obsidian Sync byte-identical re-touch:** Sync's conflict resolution sometimes re-touches a file without changing its bytes — mtime updates, content identical. An mtime-keyed check would falsely flag this as a user edit and re-process. (Same issue ADR 0046 §6 hit with the dedup ledger; it solved it by keying on content SHA-256. Same solution applies here.) (b) **Obsidian plugin reformatting:** the Tasks plugin can rewrite a checkbox line cosmetically (whitespace, emoji ordering) without changing semantics. Hash-keyed: still detected as an edit and reconciled. mtime-keyed: also detected, but as a false positive — we'd re-run the normalize pass on every cosmetic Obsidian touch. The cleanest invariant is: **after every projection write, record `file_hash = sha256(bytes)` in DB; on a watcher event, recompute the hash and compare. Hash-match short-circuits the entire pipeline.** SHA-256 is ~50µs per kB; absorbed by the 2s debounce window. The ordering matters too — pre-record the hash BEFORE the OS rename, so the watcher's first event for that file already finds the hash in place. **Generalization:** for any file-based reverse-sync loop, content-keyed dedup beats mtime-keyed dedup whenever the upstream can rewrite-without-semantic-change. mtime is metadata; content is the thing.

- **Finding 2 — Two-phase commit at capture-time goes DB-first, file-second, DB-seal-third. File-first creates a watcher race that's a nightmare to debug.** When the worker projects an entry to disk, three candidate orderings exist: (a) **file-first**: write the .md, then INSERT the DB row referencing it. Failure: orphan file looks like a user edit to the reverse-watcher because no DB row matches. (b) **atomic both**: rejected — SQLite + filesystem is not atomic; no such primitive exists. (c) **DB-first, file-second, DB-seal-third**: INSERT the entry stub → write the .md → UPDATE the stub with `file_path` + `file_hash`. Failure modes are recoverable (nightly sweep for stuck queue rows; reverse-watcher discovers files with no DB seal data and fills them in). I picked (c). The non-obvious bit is *why* (a) is worse than the orphan-file failure cost it appears to have: the reverse-watcher is going to see EVERY file we write, including the ones the worker just wrote. If the DB row isn't there yet (option a), the watcher can't distinguish "new user file appeared via Obsidian Sync" from "file I just wrote and haven't recorded yet." Option (c) lets us pre-record the hash BEFORE the watcher event arrives → loop-prevention check fires no-op → race window collapses to zero. **Generalization:** when a watcher subsystem and a writer subsystem share a substrate (filesystem), the writer should establish its own identity in the loop-prevention ledger BEFORE producing the artifact, not after. Otherwise the watcher's first observation of the artifact races with the ledger's first observation of it.

- **Finding 3 — Filenames in user-facing markdown projections need a durable-identity suffix even when YAML frontmatter has a stable `id` field.** The temptation is to make filenames purely cosmetic (`<title-slug>.md` or `<date>-<slug>.md`) and rely on the YAML `id:` for identity. Two problems. (a) The reverse-watcher's identity-recovery path needs a fallback when the user accidentally strips frontmatter (e.g. selecting-all + retyping the body in Obsidian, copy-paste replacing the whole file). With a suffix like `__<id8>` (8 chars of ULID), the watcher can recover identity from the filename alone even when frontmatter is corrupted. (b) Filename collisions are real: two `kg-note`s dictated on the same day with similar titles produce the same date-prefixed slug. Obsidian doesn't auto-disambiguate filenames; the second write would clobber the first. The `__<id8>` suffix is 40 bits of collision resistance — fully resolved. ADR 0046's history projection landed on the same suffix shape (`2026-05-27-1408__a4f7c2d3.md`) for the same reasons. The two-underscore separator is the contract; one underscore conflicts with user slugs. **Generalization:** any file projection of a DB row should embed an opaque identity suffix in the filename, even when richer identity exists inside the file. Filenames survive frontmatter corruption; frontmatter doesn't survive a careless copy-paste.

- **Action (for 1E.5 author):** the J1 judge mechanically enforces Finding 1; the J2 + J3 + worker tests gate Finding 2's ordering; the Wave 1E.2 serializer brief should hard-code Finding 3's `<YYYY-MM-DD>-<slug>__<id8>.md` shape with a property test that two distinct entries on the same date+title never produce the same filename.

---

## 2026-06-04 [KG Phase 1D Wave 1D.6 SEAL / mb-q2p1 / ADR 0052] Three judge-authoring shape findings; the "extend the Phase MC judge" framing collapsed to a sibling judge once the diff-vs-runtime distinction surfaced

- **Context:** Wave 1D.6 seals Phase 1D by authoring three acceptance-gate judges per ADR 0052 §"Acceptance gates". J1 `kg-source-gate-invariant` (NEW; deterministic Rust probe). J2 `kg-dictation-untouched` ("extends Phase MC's judge" per the charter). J3 `kg-graph-off-ui-tightened` (consolidates the Playwright invariant already tightened across 1D.2/1D.4/1D.5; no new code). Three findings worth keeping for future judge-authoring waves:

- **Finding 1 — Per-cell fresh DB for matrix probes makes the blame story unambiguous when something regresses.** J1 sweeps a 3 `capture_kind`s × 2 toggle states corpus = 6 cells, each driving one entry-point call and asserting `kg_filing_queue` row counts. The naive shape (one shared tempfile DB, one cell per `step`, accumulate observed counts and assert at the end) is cheaper but loses the per-cell blame story: a failure message that says "observed 3 queue rows, expected 2" tells you nothing about *which* cell mis-fired. The shape I picked (one fresh tempfile DB per cell + per-cell assertion + a corpus-total defense-in-depth check at the end) costs 6 `NamedTempFile::new()` + 6 `apply_all()` invocations — ~hundreds of ms total — in exchange for failure output like `corpus cell \`kg-note + toggle on\` produced 0 kg_filing_queue rows (expected 1)`. Same diagnostic shape Phase MC's `mc-formatter-deterministic` uses for its per-fixture loop. **Generalization:** when authoring a matrix-shaped invariant probe, per-cell isolation beats per-cell speed; the cost of one more DB open is rounding error against the cost of a future engineer not knowing which cell broke.

- **Finding 2 — "Extend judge X" charter text often collapses to "author a sibling judge to X" once you draw the diff-vs-runtime distinction.** ADR 0052 §"Acceptance gates" J2 said "extends Phase MC's `mc-dictation-untouched` judge" and added "any other diff in `dictation/*` or `dictation.rs` between `phase-mc-start` and HEAD = fail" as the new assertion. But Phase MC's judge is a **diff judge** — it asserts file paths in a commit range — and Wave 1D.1 already legitimately added the source gate to `dictation.rs`, which would trip the literal diff-judge assertion. The honest framing is: Phase MC's judge cannot be "extended" to accept this delta without effectively deleting its principal assertion. The right shape is a Phase 1D **runtime twin** of MC's judge — same name root, different module, asserting the runtime behavior (standard dictations produce zero `kg_*` writes) that is the durable invariant once you allow the file itself to be modified. The Phase MC judge stays as-is for its commit range; the Phase 1D judge picks up the invariant from `phase-mc-complete` forward in a way that's robust to authorized file-level surgery. **Generalization:** when a charter says "extend judge X" but X's assertion shape is incompatible with the new state of the world, the authentic answer is usually a sibling judge that picks up X's invariant in the new shape — not torturing X's prose to swallow the inconsistency. The diff-vs-runtime distinction is the most common axis along which this collapse happens.

- **Finding 3 — A "no new code shipped" judge is a legitimate shape; documenting a consolidated invariant set is its own work.** J3 `kg-graph-off-ui-tightened` was originally framed in the kickoff as the consolidated outcome of three prior waves' tightenings (1D.2 KG screen walk, 1D.4 Dictations KG-free assertion, 1D.5 vocabularies allowlist). When I sat down to author it, the natural impulse was "add MORE assertions to the Playwright spec" — but there was nothing left to assert; the spec was already complete. The right output was a judge doc that **enumerates the consolidated invariant set as fully satisfied** and pins the criteria + last-run evidence, so a future regression to ANY of the three component invariants surfaces with a clear pointer to which wave originally authored it. The doc IS the deliverable; not every judge ships a new spec file. **Generalization:** when a charter promises a judge for a multi-wave consolidation, check whether the invariant set is already executable before adding code. If it is, the judge doc that consolidates + documents it is the work; the cargo-cult "every judge needs a new spec file" instinct is wrong.

---

## 2026-06-04 [KG Phase 1D Wave 1D.4 / mb-6hm2 / ADR 0052] Subtractive relocation wave; dead-state sparse columns are an anti-pattern; structural invariants beat runtime guards

- **Context:** Wave 1D.4 relocates the KG retrieval surface (filter chips + per-row chip strip + concept modal) from the Dictations page to the new `/knowledge-graph` route, and adds click-to-retry on the FlaggedBand. Dictations.tsx is now KG-free at the import-graph level; the J3 invariant (`kg-graph-off-ui-untouched`) is structurally true on that page regardless of toggle state. The wave was almost entirely subtractive on the Dictations side (-~400 LoC across 4 files) and additive on the KG side (+~280 LoC new `Retrieval.tsx` band + ~100 LoC Dashboard changes). Two findings worth keeping:

- **Finding 1 — Dead-state sparse columns are an anti-pattern; remove them from the wrong surface entirely instead of "documenting the sparse-population semantic".** The kickoff posed a (a)/(b) decision call on the category badge that Wave 1D.1's `sessions.category` column had added to the Dictations rows (`mb-oji5`). Option (a): remove the badge from Dictations entirely. Option (b): keep it but document that it's NULL on standard `dictation` rows and only populated on `kg-note` rows. The kickoff's "my read" was (a); inspection confirms (a) is the only defensible answer once you draw the actual distribution: `sessions.category` is written by the KG filing worker, which only runs on `capture_kind IN ('kg-note', 'kg-note-text')` rows (Wave 1D.1 three-gate cascade). The Dictations page lists rows filtered `WHERE capture_kind != 'kg-note-text'` AND (Wave 1D.3 result) text notes never appear there. Of the remaining rows, `capture_kind = 'kg-note'` rows DO appear on Dictations (they're audio captures with a side effect, dual-listed). But `kg-note` rows are a tiny minority — the vast majority of Dictations rows are `capture_kind = 'dictation'`, for which `sessions.category` is structurally NULL. Rendering a column that's NULL for ~95-99% of a list is a worse UX than showing nothing; the column belongs on the KG screen where its population semantic is the default expectation. **Generalization:** when a UI field is structurally NULL for the entire (or near-entire) population of a surface, that field doesn't belong on that surface. "Documenting the sparse-population semantic" is a code smell — it means the field is on the wrong surface and the doc is compensating for the misplacement. Move the field to where it's populated; don't add an asterisk explaining why it's missing.

- **Finding 2 — Structural invariants beat runtime guards: when you can turn a runtime invariant into a structural one via dead-code elimination, prefer that over a runtime guard.** The graph-off-UI invariant judge (J1, ADR 0051; J3 tightening, ADR 0052) had two possible shapes for the Dictations page post-1D.4: (i) keep the KG plumbing on Dictations but gate every `kg_*` IPC call behind a runtime `if (toggleOn && onDictationsPage)` predicate, then write a Playwright spec that exercises every conceivable interaction with the toggle in both states to prove no IPC ever fires; or (ii) remove the KG plumbing from Dictations entirely so the page module has zero `kg_*` imports, making the invariant structurally true at the bundle/import-graph level — the Playwright spec becomes a regression net rather than the primary enforcement. Wave 1D.4 picked (ii): Dictations.tsx, DictationsList.tsx, and DictationsDetailPane.tsx now have no `kg_*` imports at all. The `kg-graph-off-invariant.spec.ts` Walk 4b still asserts the runtime behavior (snapshot the spy set before+after a Dictations visit with toggle ON, assert empty diff), but the assertion is now defending against future regressions rather than carrying the load of proving the current shape correct. **Generalization:** runtime guards are necessary when the invariant has to hold across a state machine that the module legitimately participates in. They're a smell when the module can be cleanly factored to not participate at all. "Module M MUST NOT call API X under condition C" is much harder to prove (and review, and regress-test) than "module M imports no symbol from API X". Prefer the import-graph-level invariant whenever the surface allows it.

- **Bonus discovery:** the prior session had already done the `pages/{ConceptModal,DictationKgChips,DictationsFilterBar}.tsx` → `routes/knowledge-graph/{ConceptModal,EntryChips,FilterBar}.tsx` `git mv` renames as part of an earlier sub-component reorganization. They were staged in the index but the wave's expected import-target paths landed on them naturally. Always `git status --porcelain=v1` before starting a relocation wave; the relocation might be half-done already.

---

## 2026-06-04 [KG Phase 1D Wave 1D.3 / mb-0gt6 / ADR 0052] Orphan-recovery pattern for API-error-truncated dispatches; the "refactor seam" the phase doc flagged turned out not to exist

- **Context:** Wave 1D.3 ships the KG dashboard capture surface (audio note + text note lanes). The kickoff explicitly warned of a prior API-error-orphaned dispatch and asked Bernard to "check for orphaned work" before writing anything new. That warning was load-bearing: `git status` showed 11 modified files + 1 untracked (`src-tauri/src/kg/ingest_text.rs`, 421 LoC of complete module+tests) with zero new commits since the Wave 1D.2 seal (`37feed7`). Three findings worth keeping:

- **Finding 1 — When a kickoff warns of orphaned work, treat `git diff --stat` against the last seal commit as the FIRST tool call after `git status`.** The orphan had completed the high-blast-radius work (the new `kg::ingest_text` module, the runtime `next_start_is_kg_note` flag plumbing, the `dictation_start_kg_note` IPC, the integration test fixture updates, all 9 backend file diffs). What it had NOT done was the UI piece (`CaptureBand.tsx` + CSS) and one small backend filter (`commands/sessions.rs::list_sessions` had no `WHERE capture_kind != 'kg-note-text'` clause — text notes would have leaked into the Dictations history view, violating acceptance gate #2). Pattern that worked: (1) `git diff --stat <last-seal>..` to inventory the surface area; (2) `git diff <each-file>` to confirm none of the orphan's diffs collided with what Bernard would have written; (3) finish only the gaps. **Anti-pattern:** discarding the orphan's work and re-implementing from scratch — would have burned ~500 LoC of solid module + tests + thoughtful docstrings. The orphan's `ingest_text.rs` even included a thorough "divergence from ADR 0052 §D3" docstring explaining why the simpler design (reuse `sessions` + `transcripts` + a `capture_kind` filter) beat the original synthetic-entry-id sketch — re-deriving that rationale would have been pure waste.

- **Finding 2 — The phase doc flagged a refactor ("extract `kg::pipeline::run_from_text` as a shared entry point") that, on inspection, didn't need to happen.** The phase doc's §"Refactor seam" claimed the pipeline assumed `audio → Whisper → cleanup → KG` and that text-note ingest would need to factor out a shared post-Whisper entry. The actual shape of `kg::pipeline::run_pipeline` already takes `&str` (it consumes the cleaned-transcript text, not audio); the audio path writes that text into `transcripts.text` via the dictation orchestrator, and the KG worker reads it back to call `run_pipeline`. The text-note path produces the same `transcripts.text` row from a different source (user input vs. Whisper output); from the KG worker's perspective the two paths are already indistinguishable. **No shared abstraction needed; ~100 LoC of unnecessary plumbing avoided.** General rule: when a phase doc anticipates a refactor based on "the pipeline assumes X", verify the assumption against the code BEFORE building the abstraction. Phase docs are written far enough ahead that the code has often already converged at a different seam than the doc imagines. The only real factoring needed was splitting `resolve_active_mode_from_db` into a `_from_conn` variant so the text-note path could reuse it without re-locking the DB mutex it already holds — that's the kind of small surgical seam discovery makes obvious; the bigger one didn't materialize.

- **Finding 3 — Clippy's `doc_lazy_continuation` lint reads `+ word` at the start of a doc-comment line as a list-item marker.** The orphan's docs had two paragraphs that, when wrapped, broke at lines starting with `+ clears`, `+ text`, etc. Clippy refused them as malformed list continuations. The fastest fix is to rephrase so the wrap doesn't land a `+` (or `-`, or `*`) at the start of a line — "Read AND cleared at...", "audio AND text", etc. Indenting the continuation with two spaces (`/// + clears...` → `///   + clears...`) is the lint's suggested fix but reads as a literal bullet in the rendered docs, which is wrong here. Watch for any of `+ -, *` at the start of a wrapped doc line; rephrase rather than indent.

---

## 2026-06-04 [KG Phase 1D Wave 1D.1 / mb-pxzk / ADR 0052] Column name collision avoided; live drift purge confirmed; three-gate cascade pattern

- **Context:** Wave 1D.1 ships migration 025 (two new `sessions` columns + composite index + dev-drift purge + toggle reset, all in one transaction) and reverses the dictation-tail KG filing trigger direction — only `capture_kind='kg-note'` sessions enqueue when the toggle is on; the toggle no longer auto-files every dictation. The cargo gate ran green (fmt/clippy/test --release --no-run/binary probes) in one pass after the test-fixture column plumbing was threaded through. UI gate clean (no UI changes this wave). Three findings worth keeping:

- **Finding 1 — When the kickoff text proposes a column name, grep the existing schema for it FIRST.** The Wave 1D.1 kickoff said "ADD new `sessions.source` column" with values `'dictation'` / `'kg-note'` / `'kg-note-text'`. But `sessions.source` ALREADY exists from migration 018 (ADR 0046) with values `'desktop'` / `'desktop-import'` / `'mobile-inbox'` (the audio-origin axis read by the export-job filter). Reusing the name would have either (a) demanded dropping migration 018's column (DB-destructive lateral epic) or (b) overloaded one column with two unrelated semantic axes — durably breaking the "one column, one meaning" rule the codebase relies on for `parse_db` / defensive-default parsing. **Picked `capture_kind` instead** and documented the divergence in the migration SQL header + the `CaptureKind` enum docs + this LESSONS entry. Rule: when a kickoff proposes a column name, `grep -rn "sessions\.<name>\|ADD COLUMN <name>" src-tauri/src/db/migrations/` before the first ALTER TABLE. The kickoff is a wave-author talking through intent, not a name-prescription.

- **Finding 2 — A migration that includes a destructive purge of derived data should live-apply against the actual user DB as part of the wave, with a backup taken first.** The migration 025 dev-drift purge (4 tables × ~19 rows estimated; actual was 19) was authorized in-scope by the charter ADR, but the only way to *verify* the purge worked end-to-end is to run it on the real DB. Pattern that worked: (1) `Copy-Item` the `.db` + `.db-shm` + `.db-wal` into a backup subdir under the same `%APPDATA%` folder; (2) write a throwaway `src/bin/migrate_user_db_pxzk.rs` that opens the DB raw to print pre-state, then calls `Database::open()` (which runs `apply_all` + integrity check), then prints post-state + asserts (schema_version=25, all four kg_* tables empty, toggle=false, both new columns present, all existing rows backfilled to `capture_kind='dictation'`); (3) run the binary; (4) delete the throwaway. Pre-state revealed 3+3+11+2 = 19 drift rows + toggle=true; post-state was clean across the board with 129 sessions intact. **The throwaway-binary recipe is a good pattern for any one-shot DB op** — Cargo `[[bin]]` discovery is automatic, lib re-export gives access to `Database::open()`, and the binary self-documents the verification logic the way an ad-hoc REPL session does not.

- **Finding 3 — When adding a third gate to a multi-gate helper, extend the existing invariant probe to sweep the new gate's value-space too, even if it doubles the call count.** `try_enqueue_for_kg_filing` had two gates (outcome, toggle) and the `kg_graph_off_invariant` probe swept all 8 `InjectionOutcome` variants × KgGraphEnabled=false plus a positive-control flip. Adding the source gate meant the probe could have either (a) kept the same 8-variant sweep with `CaptureKind::Dictation` only (the safe default, but leaves `KgNote × toggle=off` unprobed) or (b) doubled to 16 sub-checks. Picked (b) because the whole point of the new gate is defense-in-depth: `KgNote + toggle=off → no enqueue` is the load-bearing invariant, and it's exactly the cell the original probe didn't cover. Also added a source-gate negative control (toggle on + Dictation + Ok → no enqueue) to assert the new gate is doing work even when the other two would have permitted. Cost: ~10 extra LoC + ~50ms probe runtime. Benefit: the binary report now reads "all 8 InjectionOutcome variants × (Dictation, KgNote) left every kg_* table empty" — one line, both gates exercised, no ambiguity. General rule: a multi-gate cascade's regression probe should sweep the cross-product of independent gates, not just one gate's value-space.

---

## 2026-05-30 [KG Phase 1C Wave 1C.4 / mb-sx6p / ADR 0051] Concept-modal wave shipped clean; three small findings worth keeping

- **Context:** Wave 1C.4 added the entity/tag concept page (modal-in-v1 per ADR 0051 §D4) on top of the 1C.3 chip strip + filter bar. Two new IPCs (`kg_entity_detail`, `kg_tag_detail`) + two store helpers (16/16 throwaway-crate tests green) + `ConceptModal.tsx` (332 LoC, well under the 600 budget) + Dictations.tsx hooked up state for `activeConcept` + DictationKgChips chips became `<button>` when an `onConceptOpen` callback is passed. ui-author landed it in one pass; all four cargo gates green; UI gates (tsc + vitest 140/140 + build) green; Playwright sweep for the new modal 1/1; pre-existing kg-dictations-retrieval + kg-failed-filings still green after a one-line locator update. Three findings:

- **Finding 1 — When a previously-inert element becomes interactive, its aria-label MUST change, which WILL regress prior specs that locate it.** The 1C.3 chip strip rendered each chip as `<span aria-label="Entity Mom">` because the chip was a passive display element. 1C.4's whole job was to make chips click-to-open-the-modal, so the chip became `<button aria-label="Open concept page for entity Mom">`. The aria-label change is correct (an interactive element's label should describe the action, not just the data — "Entity Mom" alone reads to a screen reader as a stray noun on a focusable element). But the 1C.3 spec located the chip via `getByLabel(/^entity mom$/i)`, which is now a stale locator that no longer matches anything. **Fix was a 2-line edit + a 9-line code comment explaining the drift to the next reader.** The general lesson: **when wiring up a previously-display-only element to handle a user action, expect to update every spec that locates it.** Grep `getByLabel\(.*<entity-name>` and `getByRole.*<element-shape>` at wave-plan time — the locator surface area is the test-debt surface area. The fix is mechanical; the cost is forgetting to look.

- **Finding 2 — Clippy `doc_lazy_continuation` reads `///` lines that start mid-sentence as a markdown list continuation if they look list-like.** `entries.rs` had a docstring like `"... is **always None in 1C.4** —" / "the pipeline parses ..." / "+ a worker write; ..."`. The em-dash at end-of-first-line + the literal `+` at the start of a continuation line (meant to be a typographic plus-sign in body prose) tripped clippy into thinking the comment was a malformed bullet list. **Fix is purely cosmetic:** reflow into a single parenthetical sentence (`(the pipeline parses ... -- needs a sessions-table column + a worker write, not in 1C.4 scope)`) so no continuation line starts with a punctuation glyph that looks like list-bullet syntax. **Rule of thumb for module-level `///` prose:** don't start a continuation line with `+`, `*`, `-`, or a number-followed-by-`.`/`)`. If you need a bulleted list, indent it with two spaces so clippy sees it as an intentional list.

- **Finding 3 — End-of-iteration Playwright sweep surfaced 5 pre-existing failures (4 in smoke.spec.ts, 1 in kg-settings-tab.spec.ts) that pre-date 1C.4 and are unrelated to its deliverables.** Triage discipline: I ran the new spec in isolation first (passed), then the broader suite (5 failures), then bisected by git-blaming each failing spec (`git log --oneline -5 -- <spec>`) to confirm none of them had been touched in 1C.4's commit range. Smoke failures: sidebar `getByRole("navigation", { name: /primary/i })` not found, `getByText("Mockingbird")` matches both `<title>` and the brand `<span>` in strict mode, `recording.html` missing its long-gone `Wave B placeholder`, design-tokens spec timing out on the missing nav. kg-settings-tab failure: an unscoped `page.getByRole("status")` matches BOTH the "Indexing in progress" notice the spec is asserting on AND the `<p role="status">No failed filings — all good!</p>` empty state that landed in Wave 1C.2 — i.e. a 1C.2-era regression, not a 1C.4 one. **Filed as mb-26aw (P3); not gating 1C.4 on it.** The general lesson: **end-of-iteration Playwright sweeps will surface failures that have nothing to do with the wave's work.** The right move is bisect → bead → carry on, not panic-fix in-scope (which inflates the diff and obscures what 1C.4 actually shipped) and not silently ignore (which lets the failure list grow until the suite is useless as a regression signal). The kickoff brief's stop conditions explicitly list 1C.4-scope failures; pre-existing failures aren't on that list.
...
- **Finding 3 — `SUM(CASE WHEN ... THEN 1 ELSE 0 END)` over an empty SQLite table returns `NULL`, not `0`. Use `COUNT(CASE WHEN ... THEN 1 END)` for safe "how many of state X are there" counters.** The first cut of `queue_status(conn)` in `kg/store/queue.rs` did `SELECT SUM(CASE WHEN state='pending' THEN 1 ELSE 0 END), SUM(...), SUM(...), MAX(...)` and unpacked into `(i64, i64, i64, Option<String>)`. cargo `check` + `clippy --release` both pass cleanly — this is a runtime behaviour, not a type error. On an empty `kg_filing_queue`, SQLite's SUM-over-no-rows returns NULL (it's an aggregate of an empty set, and SQL's standard answer is NULL — `SUM(0)`-or-similar reasoning is wrong; the CASE expression is never evaluated because there are no rows). rusqlite then explodes at decode time: `InvalidColumnType(0, "SUM(CASE WHEN state = 'pending' THEN 1 ELSE 0 END)", Null)`. **The fix is `COUNT(CASE WHEN ... THEN 1 END)`:** COUNT counts non-NULL values, NULL arguments to CASE-without-ELSE are filtered out, and COUNT-over-no-rows always returns 0 (never NULL). Same semantics, never-NULL return. **Caught by the throwaway-crate recipe (LESSONS P2), not by cargo check/clippy/test-no-run.** This is exactly the class of bug that linker-only gates miss: clean types, clean traits, clean release link, but the actual query is wrong. The throwaway-crate took 50s to set up + one 148s test run to surface the bug; the fix + re-test was another 7s. If I'd skipped the throwaway and shipped on the linker-only gate, the qa-kitten Playwright sweep would have surfaced it as an unhelpful `unwrap()`-panic in the IPC bridge — much worse triage cost. **Action:** for any new pure-Rust store-layer additions whose tests would otherwise run via `cargo test --release` (currently blocked by the STATUS_ENTRYPOINT_NOT_FOUND issue), the throwaway-crate recipe is not an optional second pass — it IS the live test gate. The crate setup is a fixed cost (~50s); the marginal cost of running 15 tests is sub-second. Worth it.
...
- **Finding 1 — `capabilities/default.json` is wildcard (`core:default`); per-command allowlisting is a non-issue on this surface.** The kickoff brief (and ADR 0051's wave plan) flagged "register both commands in `capabilities/default.json` — this is the #1 historical regression per ADR 0035". When I went to make the edit I discovered the file uses the `core:default` permission set, not a per-command allowlist. The ADR 0035 / `mb-z5y` historical regression was actually about a missing `"windows":` array entry (a platform-targeting omission), not per-command registration. Net: new `#[tauri::command]` functions are exposed to the UI as soon as they're added to `tauri::generate_handler!` in `commands/mod.rs`; no capabilities edit needed for command registration alone. **Action:** when a future ADR says "register in capabilities", verify it's actually per-command or platform-array before adding the edit. Grep `capabilities/default.json` for `"identifier":` entries — if there are none, the file is wildcard.

- **Finding 2 — qa-kitten's tooling environment in this Code Puppy install lacks fs-write; route new spec authoring to code-puppy.** Dispatched qa-kitten to author + run `ui/tests/kg-settings-tab.spec.ts`. It returned a partial-complete with a well-drafted spec in its report body but couldn't write the file. Authored the spec inline + ran it from code-puppy in one extra turn. **Action:** qa-kitten is the right pick for *running* visual sweeps and *judging* screenshots; for *authoring* new `.spec.ts` files use code-puppy (or a JSON agent with file tools) and have qa-kitten review the output. This may relax in a future Code Puppy build; for now treat "author + run" as a code-puppy task and "judge + diff screenshots" as the qa-kitten lane.

- **Finding 3 — boot-vs-poll promotion via a per-tick settings read costs ~1 SQL per IDLE_SLEEP and the math is fine.** Worker now spawns unconditionally and reads `KgGraphEnabled` at the top of every drain loop tick. Previously the gate lived in `lib.rs::run()` at boot, so flipping the setting required an app restart. The per-tick poll adds one indexed SELECT every IDLE_SLEEP (5s) when the toggle is OFF — negligible. Three unit tests in `kg/worker.rs` lock the contract: missing settings table → false (fail-closed), missing row → false (default-fallback), and reflect-flips-from-the-DB. Pattern generalizes to any boot-gated worker that should be runtime-toggleable: keep `spawn` unconditional, move the gate inside the drain loop, fail-closed on poll error. **Action:** future KG settings additions in 1C.2+ that should also be runtime-toggleable should follow this pattern rather than adding new boot-time conditionals.

---

## 2026-06-03 [KG Phase 1B Chunk 5 SEAL / mb-k17a / ADR 0050] Two probe-shape findings worth keeping for Phase 1C+

- **Context:** Chunk 5 sealed Phase 1B by extending the `kg_parity` binary with a `--persist` mode + authoring a new `kg_graph_off_invariant` binary. Both went green first try (0 retries, ~25 minutes including the seal-commit-artifact authoring). Two structural observations that future probes can inherit:

- **Finding 1 — a one-shot trigger-fire gate decouples cleanly from the per-fixture loop.** Migration 024 ships two `BEFORE UPDATE` immutability triggers (`kg_entity_mentions` + `kg_tag_mentions`) that the store-layer round-trip never exercises in production (the layer only INSERTs / DELETEs). To verify the triggers fire, the natural-feeling option is to add a per-fixture trigger probe inside the round-trip; the better option is a single end-of-run trigger gate that opens its own tempfile + sessions + entity + mention rows and asserts the UPDATE fails with the expected RAISE message. Triggers are a global, schema-level property; running per-fixture would 32x the cost without adding signal. **Generalization:** when an invariant lives at the schema/trigger layer (not the per-row layer), gate it once per run, not once per fixture.

- **Finding 2 — a positive-control flip turns a vacuous-pass into a structural assertion.** The `kg-graph-off-untouched` invariant says "every kg_* table is empty when KgGraphEnabled=false". The naive shape is "sweep all outcomes, assert 0 rows". The trap: this passes vacuously if a regression breaks the helper entirely (e.g. an early `return ();` before the settings read). The positive-control flip (set the toggle ON, fire one Ok outcome, assert 1 row in `kg_filing_queue`) costs ~3 extra lines but proves the helper IS structurally able to write — it just doesn't. Same shape as the Phase MC `mc-dictation-untouched` probe's enable-flip. **Generalization:** every "feature-off must be a no-op" invariant probe should include a feature-on positive control. The cost is trivial; the regression-detection power is not.

- **Action / generalization:** Phase 1C / 1D / 1E retrieval + backfill probes should inherit (a) the one-shot-trigger-gate pattern for any new schema-level immutability invariant and (b) the positive-control flip for any new opt-in-feature gate. Both patterns are now ~20-line conventions, no library helper needed.

- **Bonus finding (tempfile dep promotion):** Promoting `tempfile` from `[dev-dependencies]` to `[dependencies]` is the right call when the project's `#[test]` runner is broken (LESSONS P2). The two Chunk 5 probes are first-class binaries shipped in the library; pretending they were dev-only would have forced them into `tests/` (where they can't run anyway) or duplicated tempfile logic in the bin. Treat probes as production code when the test runner can't host them.

---

## 2026-06-02 [KG Phase 1B Chunk 4 / mb-ryq4 / ADR 0050] Chunk 3 escaped its cargo gate with a fmt regression in `kg/pipeline.rs` because rustfmt's `cargo fmt --check` wasn't actually run pre-commit

- **Context:** Chunk 4 ran `cargo fmt --check` as its first gate step and got a single-line failure in `src-tauri/src/kg/pipeline.rs` — `use super::passes::{self, Classification, Extraction, ExtractedEntity, ...}` should be alphabetized as `..., ExtractedEntity, Extraction, ...`. Bisecting (`git show 45ac718`) traced the bad ordering to Chunk 3's pipeline-5th-pass commit. Chunk 3's report-back claimed "Cargo gate green" but the regression slipped through.
- **Likely root cause:** Chunk 3 may have run `cargo clippy --release -- -D warnings` (which DOES pass on this file) without separately running `cargo fmt --check`, OR ran them together and didn't notice fmt failing because clippy's success line came after. The wrapper script doesn't chain them; each gate step is an independent invocation.
- **Detection cost:** about 1 minute (the fmt diff was a single line, easy to read). Fix cost: included in Chunk 4's commit as a sweep-up, plus a STATUS callout.
- **Action / generalization:** the end-of-iteration discipline section in AGENTS.md already lists `fmt --check` as a separate step. The lesson here is for sub-agent dispatches that produce a single "cargo gate green" report: **all four gate calls (`check`, `fmt --check`, `clippy --release -- -D warnings`, `test --release --no-run`) must be run and their pass status reported individually**, not collapsed into a single "gate green" claim. A report-back that says "gate green" without itemizing has a hidden failure mode where one of the four was skipped. **Convention going forward:** report-backs list each of the four with its own ✓ or ✗.
- **Bonus finding:** when a stray pre-existing fmt regression is detected mid-iteration, the right action is **sweep-it-up-and-note-it-in-the-commit** rather than escalating or stash-bypassing. The fix is mechanical, ownership-neutral (rustfmt would have made the same change on first edit), and small enough not to muddy a single-bead commit. Same `git blame` outcome either way.

---

## 2026-06-02 [KG Phase 1B Chunk 3 / mb-eke8 / ADR 0050] Production `run_pipeline` was 4-pass-only despite ADR 0049 §6 binding 5 passes — surfaced because Chunk 3 went to wire `extract_entities` consumption and discovered nobody produced it

- **Context:** Chunk 3 kickoff brief authorized a worker thread + boot wiring. While reading `pipeline.rs` to plan the worker's `apply_filed_outcome` data shape, discovered the production `run_pipeline` only runs 4 passes (segment → classify → extract → normalize-via-assembly). The Wave 0.5.4 entity probe ran `extract_entities` standalone for its validation snapshot, and the parity probe (`kg_parity`) STILL runs it standalone post-`run_pipeline` (see `parity.rs` line ~410). But no production caller ever invoked it. Chunk 2's `kg_entity_mentions` table was thus dead-on-arrival — the worker would have populated `kg_tag_mentions` only.
- **Finding 1 (the gap):** ADR 0049 §6 binding ("v1 pipeline = 5 passes including `extract_entities`") + PHASE-0-5-REPORT.md §6 was being silently violated by Phase 1A's graduation, because Phase 1A's parity probe asserts only on `(entries, per_pass_errors, new_tag_requests)` — entity outputs ride in a SEPARATE assertion path that bypasses `run_pipeline` entirely. Coverage gap: there was no test that would have failed when `extract_entities` was missing from the orchestrator. The Chunk 2 LESSONS entry (above) correctly noted that the additive-field path was safe; the unspoken assumption was that the field would be populated. It wasn't.
- **Finding 2 (additive-field-invisibility CONFIRMED EMPIRICALLY):** I extended `PipelineResult` with `segment_entities: Vec<SegmentEntities>` and wired the 5th pass into the per-segment loop. `kg_parity` stayed 32/32 GREEN — confirms Chunk 2's manual-JSON-builder hypothesis at zero risk. This is now load-bearing precedent for ADR 0050 §6 and beyond: structurally-additive `PipelineResult` extensions are parity-safe by construction. **If a future change ever switches `pipeline_result_to_value` to `serde_json::to_value(&result)`, that's a parity-contract change and must be flagged at review.**
- **Finding 3 (MockOllama needle ordering for the 5th pass):** the `extract_entities` prompt suffix is `\n\nSEGMENT:\n{seg}\n` — IDENTICAL shape to `classify`. Disambiguation must come from the **prompt body**. The classify body and the extract_entities body are disjoint, so a body-substring needle works, but it MUST go ahead of the SEGMENT-keyed classify needle in the rule chain (`MockOllama` is first-substring-match-wins, not most-specific-wins). Unique opener `"You extract specific named or concrete"` is in both the open-vocab and `mid-confident` profile prompt bodies, so it's a stable needle. **Same pattern as LESSONS 2026-05-28 (extract vs classify needle ordering) — generalize: every time a new pass joins the pipeline, audit ALL existing MockOllama-driven tests in the kg suite for prompt-body collision and prepend the new pass's needle.**
- **Finding 4 (decision-pre-empted-by-discovery):** the original Chunk 3 brief had option (a)/(c) for D1 still as an "open" decision per the brief §4 footnote; my Chunk 2 picked (c) on YAGNI grounds because populating `PipelineResult.segment_outputs` would have been dead code without consumers. Chunk 3 wiring made (a) suddenly live AND backwards-compatible — both store-layer `SegmentOutput` (Chunk 2 chose) and pipeline-layer `SegmentEntities` (Chunk 3 added) now coexist; the worker translates between them in `build_segment_outputs`. This is not a regret on Chunk 2's pick — it's the natural seam between "what the pipeline naturally produces" and "what the store layer wants to consume".
- **Action:**
  - When a phase claims to graduate a multi-pass pipeline, gate the seal on a test that asserts every pass is reachable from production — not just that individual passes work. The Phase 1A parity probe was technically correct (matches the documented contract) but covered insufficient surface; the entity assertions ran standalone. **Add to ADR 0050 Chunk 5 acceptance: the `kg-graph-off-untouched` judge should be paired with a `kg-pipeline-passes-all-runtime-reachable` probe.** [add as bead at Chunk 5 anchor]
  - For Chunk 4 (dictation tail surgical edit): the worker's input shape is now fixed — `entry_id` is a `session_id`, the worker loads `transcripts.text` (final > cleaned > raw), `sessions.started_at` becomes `captured_iso`. The dictation hook just needs to call `enqueue_for_filing(conn, session_id, now_iso)`. Two enqueue points to consider per ADR 0045 split-mode: `InjectionOutcome::Pasted` (PTT path) AND `InjectionOutcome::InAppNoInject` (in-app path). Both have a finalized session row + a `final`-stage transcript by the time injection resolves, so both can enqueue.
  - Keep the additive-field rule alive: if Chunk 4 or 5 needs to thread MORE per-segment data through `PipelineResult` (e.g. per-segment confidence scores, per-segment timings), do it the same way and re-confirm 32/32 GREEN as a one-shot empirical check. Don't re-derive the invisibility argument every time.

---

## 2026-06-01 [KG Phase 1B Chunk 2 / mb-geds / ADR 0050] `PipelineResult` is structurally extensible at zero parity-risk because the parity probe uses a manual 3-field JSON build-up

- **Context:** Chunk 2 needed to decide whether to thread per-segment provenance (D1) into `kg::PipelineResult` (option a per the wave brief) or define a store-layer-only `SegmentOutput` type (option c, my pick). The blocker was the supposed parity risk — if I added a field to `PipelineResult` and the parity probe re-serialized the whole result, the new field would land in the JSON and break byte-identical equality with the fixture.
- **Finding:** `parity.rs::pipeline_result_to_value` does NOT use `serde_json::to_value(&result)`. It manually builds `json!({ "entries": entries, "per_pass_errors": per_pass_errors, "new_tag_requests": new_tag_requests })` from three field accesses. `PipelineResult` itself is `pub struct PipelineResult { ... }` with **no `#[derive(Serialize)]`** — adding fields is undetectable to the parity probe by construction. The wave brief's R3 framing assumed serde-driven serialization where `#[serde(skip)]` would be load-bearing; the actual code doesn't need that gymnastics.
- **Action:**
  - When evaluating whether a struct extension is parity-safe, **check whether the serialization path is serde-derived or manually-built before reaching for `#[serde(skip)]`**. The current parity code is the source of truth, not the assumption that "serialize" means "automatic".
  - For Chunk 2 specifically I still picked option (c) — store-layer-only `SegmentOutput` — for YAGNI reasons (Chunk 2 has no production data flowing through a `PipelineResult.segment_outputs` field; populating it empty would be dead code; Chunk 3 decides population locus). But the parity-risk argument that pushed option (a) into a "refined" status is moot.
  - If Chunk 3 wants to wire `extract_entities` INTO `run_pipeline` and surface segment-keyed entity data via `PipelineResult`, it can do so without worrying about parity. Just add the field; the manual serializer will ignore it.

---

## 2026-05-31 [KG Phase 1A Chunk 3 SEAL / mb-2mc9 / mb-cskk] Parity gate caught two independent fixture-vs-reality bugs that only surfaced on the corpus's one parse-failure dictation

- **Context:** Phase 1A graduation gate is a bit-identical parity probe (`src-tauri/src/bin/kg_parity.rs`) that re-runs the production `kg::run_pipeline` against the Wave 0.5.4 seed-42 fixture via a fixture-scripted `OllamaDispatcher`. First green-run attempt landed 31/32. The lone failure (persona-06-case-05) turned out to expose **two independent** fixture-vs-reality drifts. Worth recording because the natural failure mode would have been to relax the probe — the correct fix is to make the fixture honest.
- **Failure shape (first attempt):** all 32 fixtures' aggregate-entity comparison flagged `segment_failures` mismatch (production = `[]`, fixture = real sandbox per-segment failure log).
- **Bug A (probe-side):** the probe's `entity_objects_set_equal` asserted on `segment_failures`. But that field is **sandbox-run provenance** — it documents which segments hit JSON-parse failures during the original sandbox capture. It cannot be reproduced from aggregate canned responses (the canned response always parses cleanly because it IS the parsed object re-serialized), so production runs always have empty `segment_failures`. `parity/README.md` §3 option 1 ("Aggregate-only assertion") had already pinned this: assert at the `{name, type, aliases}` set level, not on provenance metadata.
- **Fix A:** dropped `segment_failures` from the comparator. `segment_count` stays asserted (it IS reproducible — it's just the input segment count). Added a unit test (`entity_objects_set_equal_ignores_segment_failures_drift`) so the principle doesn't regress.
- **Failure shape (after A):** 31/32. Lone failure was `persona-06-case-05 — pipeline_result diverged`. Fixture expected `per_pass_errors: []`; production produced `[("classify[0]", "dispatcher error: MockOllama: ... no canned response (have 1)")]`.
- **Bug B (fixture-side, the smoking gun):** inspecting `experimental/kg-validation/runs/iter-1-7b-fix/raw/persona-06-case-05/classify-0.json` revealed the sandbox artifact had `parsed: null`, `error: "JSON parse failed in classify pass: unknown variant \`objective\`, expected one of ..."`, and `raw_model_output: "{\"category\": \"personal\", \"entry_type\": \"objective\"}"`. The sandbox model returned valid JSON with an enum value (`"objective"`) the schema doesn't accept. The sandbox classify pass logged a `PassError::Parse`. The fixture aggregator (`aggregate_fixture.py`) had TWO related shortcuts: (1) it only read `parsed` from each per-pass artifact, throwing away `raw_model_output` when `parsed` was null, so the canned sidecar carried `classify[0]: null` instead of the raw string; (2) it hardcoded `pipeline_result.per_pass_errors: []` for every fixture, regardless of whether the sandbox artifacts contained `error` fields. Shortcut (2) happened to be correct for 31/32 fixtures (the sandbox ran clean) — it was a lie that matched reality by luck.
- **Fix B:** patched `aggregate_fixture.py` with two helpers: `_canned_response_for(artifact)` falls back to `raw_model_output` when `parsed` is None (so production replays the unparseable string and `PassError::Parse` fires with the same message); `_collect_per_pass_errors(seg, classify, extract)` walks every per-pass artifact's `error` field and emits `[stage_tag, error_msg]` tuples. Verified that production's `PassError::Parse` Display format (`"JSON parse failed in {pass} pass: {error}\nRaw output:\n{raw}"`) is **byte-identical** to the template the sandbox writes — the round-trip is honest. Re-ran the aggregator; fixture diff was 7 added + 2 changed lines, all surgical to persona-06-case-05. Probe → 32/32 GREEN.
- **Why this is worth a lesson and not just a bug fix:** the natural temptation when a probe is 31/32 is to relax the probe (loosen the comparator, ignore the field, declare the case an acceptable outlier). That would have shipped a parity gate that swallows real production errors. The correct direction is the opposite: when the probe disagrees with the fixture, ask which side is lying. In both A and B, the fixture/probe side was the one that needed to change — Bug A because the probe was asserting on a field the architecture explicitly designates as unreproducible (the README spelled this out and the probe ignored it); Bug B because the aggregator threw away provenance the sandbox had carefully captured.
- **Pattern lesson — for future fixture-aggregator authors:** if the source artifacts contain *any* error / raw-output / failure-mode field, the aggregator MUST preserve it. "This corpus ran clean so I'll hardcode `[]`" is the bug. The fixture's job is to be a faithful snapshot of what the source actually did, not what the happy-path entries claim it did. The one weird dictation in 32 is exactly where the fixture earns its keep.
- **Pattern lesson — for future probe authors:** read the fixture's README's "what the fixture asserts vs what it does not" section BEFORE designing the comparator. Parity gates that compare more than the fixture promises to reproduce will fail spuriously and tempt the author into relaxing the gate; parity gates that compare less than the fixture promises will pass on broken production code. The README is the contract; honor it.
- **Bead trail:** discovery bead `mb-cskk` ("parity probe fails 32/32 / 31/32; aggregator + comparator fixes") created mid-iteration and closed in this seal; epic `mb-2mc9` closes via the seal commit.
- **Not PINNED:** narrow blast radius (parity-fixture aggregators are rare; the next one is at least a Phase 1B's distance away). Body entry is sufficient.

---

## 2026-05-31 [KG Phase 1A Chunk 2 / mb-2mc9 / Wave 2 graduation] `thiserror` reserves the field name `source`; pick `loaded_from` (or similar) for error-payload "where did this come from" strings

- **Context:** Graduating the sandbox schema loader to `src-tauri/src/kg/schema_loader/`. The pre-graduation `SchemaError::PromptNotFound` variant carried a single `PathBuf`; the post-graduation version needed to surface BOTH the schema-relative path that was referenced (`prompts/segment.md`) AND a human-readable description of where the loader tried to read it from (filesystem `<abs path>` vs `bundled (include_str)`). The natural field name for the second was `source` — the loader's *source of truth* for the prompt body.
- **Symptom:** `cargo check` errored on the variant declaration:
  ```
  error[E0599]: the method `as_dyn_error` exists for reference `&std::string::String`,
                but its trait bounds were not satisfied
     |
  118 | PromptNotFound { relpath: String, source: String },
  ```
  Cryptic at first glance, because the variant was syntactically fine and the error pointed at trait bounds on `String`, not at the field declaration itself.
- **Root cause:** `thiserror` treats `source` (and `from`) as **reserved field names** that hook into `std::error::Error::source()` for chained-error reporting. It expects any field named `source` to implement `std::error::Error`; `String` doesn't, so the `#[derive(thiserror::Error)]` macro emits the unhelpful trait-bound error against `String` itself rather than naming the collision.
- **Workaround:** Rename the field. We picked `loaded_from: String` (descriptive enough to read as a printf-style label in the `#[error("...")]` template). One-line fix once the cause was clear; ten minutes of head-scratching before that.
- **Pattern lesson:** When designing `thiserror` variants, treat `source` and `from` as **system-owned identifiers** — use them only when the field's value is itself a chainable `Error`. For "where did this come from" descriptive strings, pick a different word: `loaded_from`, `provenance`, `discovered_at`, `origin`. The `thiserror` docs cover this under "transparent" attributes but the gotcha is one paragraph deep; easy to miss.
- **Why not promote to PINNED:** Narrow blast radius (only bites when you're authoring a new thiserror enum with a `String` field that you happen to call `source`). Bernard sessions composing new error types regularly enough to want a PINNED reminder are rare; once you've hit it, the muscle memory is permanent. Stays in the body.
- **Bonus finding (same iteration, same graduation chunk):** `cargo fmt --check` ran clean on the original sandbox code but flagged the graduated version several times for trivial whitespace shape — the sandbox apparently never had `fmt --check` enforced. Running `cargo fmt` once post-graduation cleared everything; future graduations should fmt-pass on the first attempt now that the precedent is set.
- **Files:** `src-tauri/src/kg/schema_loader/mod.rs` (the `SchemaError::PromptNotFound` variant; commit `1c2134c`).

---

## 2026-05-31 [KG Phase 1A Chunk 1 / mb-2mc9 / mb-8thh] PowerShell 5.1 `ConvertTo-Json` silently wraps `object[]` values in nested `[ordered]@{}` as `{value, Count}` instead of bare JSON arrays

- **Context:** Building the Wave 0.5.4 seed-42 parity fixture for KG Phase 1A. The fixture is a bit-identical re-run gate (production `kg::run_pipeline` output must match the sealed sandbox output byte-for-byte). Initial implementation used PowerShell 5.1 (the project shell) to aggregate per-dictation `pipeline_result.entries` arrays from `experimental/kg-validation/runs/iter-1-7b-fix/structured/*.json`.
- **Symptom:** The output looked structurally correct in `Format-Table` but `ConvertTo-Json -Depth 100` on the assembled fixture produced `"entries": {"value": [{...}, {...}], "Count": 2}` instead of the bare `"entries": [{...}, {...}]` the production pipeline emits. Bit-identical assertion would fail on every single one of the 32 dictations.
- **Root cause:** When `ConvertFrom-Json` parses a JSON array into PowerShell, the result is an `Object[]` wrapped in a `PSCustomObject` that carries `.value` and `.Count` synthetic properties. If that array is then assigned as a *value* in a nested `[ordered]@{ key = $value }` and re-serialized via `ConvertTo-Json`, PS 5.1 reflects on the wrapper instead of treating the value as enumerable — emitting the wrapper's `value` + `Count` properties as the JSON object body. PowerShell 7's `ConvertTo-Json -AsArray` works around this, but `pwsh` is not on PATH on this box (LESSONS P6 cross-ref).
- **Workaround:** **Rewrote the aggregator in Python.** Python's stdlib `json.dump(..., sort_keys=True)` preserves list shape verbatim, has no PSObject wrapper to confuse it, and is deterministic across re-runs out of the box (`uv`-managed Python 3.13 is on PATH on this box). Result fixture went from 191 kB of corrupted JSON to a clean 63 kB.
- **Pattern lesson:** Anywhere we need to **assemble** JSON from heterogeneous on-disk inputs into a single deterministic output file (parity fixtures, snapshot tests, schema dumps), reach for Python, not PowerShell. PowerShell is fine for path manipulation, file enumeration, running cargo commands, and shape-preserving read-process-write of JSON via raw text (`Get-Content -Raw` → string surgery → `Set-Content`). It is **not** safe for assembly via `ConvertFrom-Json | mutate | ConvertTo-Json` round-trips when any value is a nested array.
- **Why not promote to PINNED:** Narrow blast radius (parity fixtures, snapshot tests). Bernard sessions composing JSON from on-disk JSON inputs are rare; sessions doing string-surgery on JSON, or invoking cargo, or running scripts, are common and unaffected. Stays in the body.
- **Files:** `docs/knowledge-graph/parity/aggregate_fixture.py` (the working Python version, idempotent and content-stable across re-runs). The earlier PS 5.1 attempt was deleted before commit.
...
## 2026-05-31 [kg-phase-1c-wave-1c.5 / mb-f4gn] Cross-check kickoff premises against disk before the first edit -- second time around for mb-oji5

- Context: Wave 1C.5 kickoff Decision A asserted mb-oji5 was a ~5-LoC fix: thread `Option<String>` through `FilingOutcome` and add `UPDATE entries SET category = ? WHERE id = ?` in `apply_filed_outcome`, citing "migration 016 has entries.category column (1C.4 verified)". Three problems on disk: (a) there is no `entries` table -- only `sessions`, and `sessions` has no `category` column; (b) migration 016 is `activity_blocks_primary_title`, completely unrelated to KG; (c) the 1C.3 LESSONS body entry (mb-5ly5 / mb-oji5, 2026-05-30, ~30 lines above this one) had already documented this exact gap and explicitly said the fix "would have required either fabricating data or a migration + worker change that's well outside the wave's scope". Plus ADR 0051 §"Out of scope" explicitly bans new migrations in 1C.
- Finding: the kickoff premise was internally inconsistent (its own ADR forbade the implementation path it specified) AND empirically wrong about the schema (no such table, no such column, no such migration). The kickoff was authored by planning-agent in a fresh session that didn't have the 1C.3 LESSONS finding loaded into working memory. Decision A was effectively a hallucination layered on top of an architectural decision someone else had already made and documented.
- Action: stopped after the session-start ritual + reading STATUS / phase brief / 1C.3 LESSONS + grepping for `sessions.*category|entries.*category` in src-tauri (zero matches). Surfaced the conflict explicitly in the iteration's opening response ("Decision A is empirically impossible as scoped") and proceeded with Path A: defer mb-oji5 to Phase 1D, seal 1C with the honest "5 of 6 axes" framing. ADR 0051's seal narrative + STATUS.md's sealed bullet + PRODUCT-STATE §3.19 all explicitly record the deferral so the next planning pass for Phase 1D inherits the correct premise ("backfill needs a new migration anyway; absorb the sessions.category column into that migration").
- Bigger picture for future agents: a kickoff that asserts a small fix ("5-LoC, 1 file") for work that has a PRIOR LESSONS entry calling it out as out-of-scope is a strong signal the kickoff has lost the prior context. The session-start ritual already prescribes reading LESSONS PINNED + recent body entries; for any in-flight epic's seal wave, the rule should be sharpened: ALSO grep LESSONS for the specific bead id the kickoff names. If the bead has a prior body entry, read it and reconcile BEFORE the first edit. The kickoff is a suggestion; the on-disk ground truth (code + prior LESSONS + ADR scope) is binding. When they disagree, the kickoff is the one that's wrong.
- Tag: planning, kickoff-discipline, lessons-meta, kg

## 2026-05-31 [kg-phase-1c-wave-1c.5 / mb-f4gn ui] Playwright IPC spy via `lib/tauri.ts::invoke` choke point beats `__TAURI_INTERNALS__` shimming

- Context: Wave 1C.5 needed a Playwright spec asserting "no `kg_*` IPC fires when KgGraphEnabled is off". Kickoff suggested monkey-patching `window.__TAURI_INTERNALS__.invoke` -- but that property is ABSENT in Vite preview mode (which is what Playwright runs against per `playwright.config.ts::webServer.command = "npm run build && npm run preview"`). Installing a fake `__TAURI_INTERNALS__` would make `isTauri()` return true, dynamically import `@tauri-apps/api/core`, and route through the real Tauri client -- now we'd need to fake the rest of the Tauri 2 callback-based invoke contract just to spy on a command name.
- Finding: the cleanest spy point isn't the Tauri-internals layer -- it's the choke point ABOVE it. Every IPC in this codebase goes through `ui/src/lib/tauri.ts::invoke(command, args)` regardless of context (Tauri shell OR Vite preview fixture mode). Adding an opt-in `window.__KG_IPC_SPY__?: (cmd: string) => void` hook there is ~5 LoC, opt-in (zero cost when no test has set the global), and works identically in both runtime modes. The Playwright spec installs it via `addInitScript` before the first navigation; reads accumulated calls via `page.evaluate(() => window.__KG_IPC_CALLS__)`. The `try { spy(cmd) } catch {}` wrap prevents a spy bug from breaking real IPC.
- Action: shipped as the opt-in choke-point spy. The same pattern is reusable for ANY future "X-off-anywhere" invariant judge (graph-off, meetings-off, activity-off, ...). Document the contract at the type declaration: spy is side-effect-only, MUST NOT alter the return value of invoke. Production cost: one `if (window.__KG_IPC_SPY__)` per IPC, negligible.
- Bigger picture: when a kickoff or ADR specifies a low-level intercept point ("hook X at layer Y"), consider whether the same observable can be captured at a HIGHER, project-owned layer with less coupling to the framework. Our code is the cheapest place to add a hook; framework internals are the most expensive. Walking UP the call stack to find the project-owned choke point usually wins.
- Tag: ui, playwright, ipc, kg, judges

## 2026-05-30 [phase-1e/wave-1e.2] YAML c-printable is the real escape predicate, not C0+DEL+C1

- Context: Wave 1E.2 hand-rolled YAML emitter for the KG Markdown serializer. First pass escaped only C0 controls (`< 0x20`), DEL (`0x7F`), and C1 controls (`0x80..=0x9F`). Property test `prop_frontmatter_is_valid_yaml` (5 fixtures + `proptest::char::any()`) caught a different class of failures: `serde_yaml::from_str` rejected our output with `READER` error "control characters are not allowed".
- Finding: libyaml's reader has a stricter check than just "C0 / DEL / C1". The YAML 1.2 spec `c-printable` set is the EXACT allow-list libyaml gates on -- everything outside it triggers the reader error BEFORE the parser even sees the bytes. Concretely, the failing chars `prop::char::any()` generated were Unicode non-characters U+FFFE / U+FFFF (valid Rust `char`s; not in `c-printable`). Also need to escape NEL (U+0085) / LSEP (U+2028) / PSEP (U+2029) -- they ARE in `c-printable` but act as line-break semantics inside a quoted scalar, so emitting them raw breaks the one-line invariant.
- Action: switched to `!is_yaml_printable(c)` as the escape predicate, with explicit NEL/LSEP/PSEP escapes layered on top. Helper documents the spec-quoted range. Escape format scales: `\x{02x}` for n <= 0xFF, `\u{04x}` for BMP, `\U{08x}` for supplementary planes. Pinned by the property test, which now exercises 1000+ generated entries on every run.
- Bigger picture: when a hand-rolled emitter for a structured format needs to round-trip through a real parser, the escape predicate must mirror the PARSER's allow-list, not just "obvious unprintables". `prop::char::any()` is the right tool to surface this -- regular fixture tests wouldn't have caught U+FFFE.
- Tag: rust, kg, vault, yaml, serializer, proptest

## 2026-05-30 [phase-1e/wave-1e.2] env!(CARGO_MANIFEST_DIR) lies inside the throwaway-crate harness

- Context: Wave 1E.2 golden-file tests use a `MOCKINGBIRD_UPDATE_GOLDENS=1` env var to write fixtures back to disk. First impl resolved the output path via `PathBuf::from(env!("CARGO_MANIFEST_DIR"))`. The throwaway-crate live-test recipe (LESSONS P2) runs the test under a synthetic crate at `$env:TEMP\md_ser_tests\`, but `env!()` is a COMPILE-TIME macro that records the manifest dir of THE CRATE BEING COMPILED -- so it expanded to the throwaway crate's TEMP path, and the goldens got written there instead of the real `src-tauri/tests/fixtures/markdown_golden/`.
- Finding: when test code needs to locate sibling on-disk resources AND will run under the throwaway-crate harness, prefer `file!()` over `env!("CARGO_MANIFEST_DIR")`. `file!()` returns the source-file path as the compiler saw it; with the throwaway harness's `#[path = "<absolute-path>"]`, `file!()` resolves to that absolute location -- which is the REAL on-disk source tree. From there, `.parent().parent().parent().join("tests").join(...)` walks back to `src-tauri/` and reaches the real fixtures dir regardless of which crate is compiling the test.
- Action: `assert_golden` now uses `Path::new(file!())`-based resolution. Confirmed the goldens land at the canonical location (`src-tauri/tests/fixtures/markdown_golden/`) and re-run cleanly without UPDATE_GOLDENS set. Sibling lesson: `include_str!("../../tests/fixtures/...")` is fine -- it's also `file!()`-relative, not manifest-dir-relative.
- Tag: rust, testing, throwaway-crate, paths, lessons-p2-followup

## 2026-05-30 [phase-1e/wave-1e.2] PowerShell Set-Content -NoNewline + array input = silent line-flattening bomb

- Context: trying to split a 1252-line Rust source file into impl + tests via PowerShell: `$lines[0..($cut - 2)] | Set-Content -NoNewline -LiteralPath ...`. Expected: write the slice with NO trailing newline after the last element. Actual: every line in the array was joined WITHOUT a separator, producing a single ~23 KB line with all source code mashed together. AND the default Set-Content encoding mangled UTF-8 multi-byte chars (em-dash U+2014 -> garbage byte sequences; section sign U+00A7 -> 0x15). The file was double-corrupted and unsalvageable; rebuilt from scratch.
- Finding: `Set-Content -NoNewline` applied to a pipeline of array elements omits the inter-element separator entirely, not just the trailing newline. The intent is "no trailing newline after the file as a whole", but the implementation drops all the separators. For multi-line file writes, this is catastrophic. ALSO: PowerShell 5.1's default encoding for `Set-Content` is Windows-1252 (codepage), not UTF-8. Non-ASCII bytes round-trip through this codepage and emerge as different bytes if any are >127.
- Action: never `Set-Content -NoNewline` with array input. For multi-line writes from PowerShell, either: (a) `[System.IO.File]::WriteAllText($path, ($lines -join [Environment]::NewLine), [System.Text.Encoding]::UTF8)` -- explicit join, explicit UTF-8; or (b) use the agent's `create_file` / `replace_in_file` tools which handle both correctly. Splitting a Rust source file? Go through `replace_in_file` to nuke the inline test block, then `create_file` the sibling test file -- DON'T script the split in PowerShell.
...
## 2026-06-08 [v0.2.0-beta.1 smoke round 2 / mb-9rgx] Reported smoking-gun bug had no observable code-side cause; the right move was static-analysis triage + diagnostic instrumentation, NOT speculative refactor

- **Context:** Round-2 dispatch reported (a) Bugs 1+2 from mb-v7pd STILL present and (b) a new Bug 4 surfaced via screenshot: "state not managed for field `db` on command `insights_snapshot`. You must call `.manage()` before using this command". Dispatch hypothesised a Tauri `app.manage(AppState)` ordering bug masking multiple boot-IPC failures and wanted me to rewire the builder.
- **Finding 1 -- End-to-end static analysis on the current HEAD (`1c9e20d`) shows no observable code-side cause for the reported error.** `AppState` is registered unconditionally at `src-tauri/src/lib.rs:137` via `app.manage(AppState::new(shared_conn.clone()))` ~12 statements into `setup`, BEFORE any fallible runtime-spawn block and BEFORE any window can be constructed (Tauri 2 creates windows after `setup` returns Ok). The tree has exactly ONE `pub struct AppState` and ONE `pub type AppStateHandle = AppState` (`src-tauri/src/commands/mod.rs:44-62`); all ~50+ command sites use `State<'_, AppStateHandle>` so the TypeId-based StateManager lookup resolves to the same type registered. Today's mockingbird.log shows setup running through `command_center WH_KEYBOARD_LL hook installed` (the last log line before `app.manage(cc)` + `tray::register`) with zero panics, errors, or warnings. The build mtime check confirmed `target/release/mockingbird.exe` (6/3 10:02) is the post-mb-v7pd binary carrying the bootApp fix + the dist (6/3 9:42). Bottom line: this code cannot produce the reported error message at runtime.
- **Finding 2 -- Bugs 1+2 subscription audit shows clean wiring: Sidebar reads from the EXACT store slices `bootApp` writes to.** Per-slot data-flow table: `activeModeSlug` & `modes` (Sidebar.tsx `useAppStore((s) => s.activeModeSlug)` / `s.modes`) <- `bootApp` `setters.setActiveModeSlug(activeModeRes.value.slug)` / `setters.setModes(modesRes.value)`. `kgGraphEnabled` (Sidebar `useAppStore((s) => s.kgGraphEnabled)`) <- `setters.setKgGraphEnabled(kgSettingsRes.value.kgGraphEnabled)`. `appVersion` (Sidebar `useAppStore((s) => s.appVersion)`) <- `setters.setAppVersion(versionRes.value)`. No mismatch between origin store slot and consumption site. mb-v7pd's pattern fix is complete and correct.
- **Finding 3 (the meta-lesson) -- When a reported smoking-gun bug cannot be reproduced from static analysis on the post-fix HEAD, the most likely cause is a stale-binary or stale-screenshot at the point of repro, NOT a deeper code bug. The first diagnostic move is to verify provenance of the repro evidence BEFORE rewriting working code.** The signs all pointed this way: (a) the report claimed "still present" but the LESSONS entry for mb-v7pd documented the prior fix shipping green gates including a fresh exe at 6/3 10:02 carrying both the Promise.allSettled fix AND the updated dist; (b) today's runtime log showed setup completing cleanly with zero state-manager errors; (c) the smoking-gun error message is a Tauri 2 String literal that CAN only be produced when `StateManager.try_get::<T>()` returns `None` for a TypeId, which requires the matching `app.manage()` call to never have been reached -- and that contradicts the live log evidence. The dispatch leaned on a hypothesis ("Tauri `.manage()` registration bug") that the code does not support. Rewriting the builder on this premise would have introduced new failure surface for zero defect-reduction.
- **Action -- Surgical, additive-only response: one `tracing::info!` line after `app.manage(AppState::new(...))` so the NEXT log will record the canonical state's registration at the canonical site; zero behavior change.** This makes the next round-3 repro (if it happens) trivially triageable from the log alone: the line either appears (=> setup did register AppState; bug is somewhere else) or it does not (=> setup did NOT reach the manage call; we now have a code path to inspect). The minimal-diff principle: do not touch correct code on a hunch; do add the smallest possible instrumentation that converts the next repro from "guess from the screenshot" to "read from the log".
- **Action (process) -- When the next round dispatch arrives with a "smoking gun" symptom, do the orphan check FIRST, then verify the runtime log + exe mtime against the prior fix's seal date BEFORE accepting the diagnosis at face value.** Round 2 here would have collapsed in ~5 minutes if the dispatch had stated "I relaunched the 6/3 10:02 exe and saw error X" -- vs. the implicit assumption that the screenshot was current. The orphan check + log/mtime cross-check is cheap; the alternative (refactoring AppState wiring on a phantom bug) is expensive and risk-creating.
- **Files touched:** `src-tauri/src/lib.rs` (+8 lines: one `tracing::info!` post `app.manage(AppState)` with site + state name).
- **Gates:** `cargo fmt --check` GREEN, `cargo check` GREEN, `cargo clippy --release -- -D warnings` GREEN, `cargo test --release --no-run` GREEN (link surface OK), `cargo build --release` GREEN (fresh exe at 6/3 16:20:29, +512 bytes for the tracing call expansion, ProductVersion 0.2.0-beta.1 stable). UI: `npx tsc --noEmit` GREEN, `npm test` GREEN (154/154 unchanged from mb-v7pd), `npm run build` GREEN.
