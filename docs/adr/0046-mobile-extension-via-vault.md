...
#### 3.2 Orchestrator input-channel topology (amendment 2026-05-27)

The headless ingest function (§3) is invoked from two callers: the Tauri "+ Audio file" IPC (Iter 1) and the inbox watcher courier flow (Iter 3). Both need to feed work into the orchestrator's thread because the orchestrator owns the heavyweight `Box<dyn VoiceActivityDetector>` / `Box<dyn SpeechToText>` / `Box<dyn Cleaner>` instances and they cannot leave that thread. Constructing fresh instances per call would duplicate the whisper-rs CUDA allocation (~3-4 GB VRAM each), risk GPU exhaustion on smaller cards, and re-pay the cold-load latency we already amortise via `runtime::spawn_ollama_warmup` — none of those costs are acceptable on a per-import basis.

The orchestrator's current input is a single `std::sync::mpsc::Receiver<StateAction>` produced by `StateDriver::start()` and consumed by `DictationOrchestrator::run` via a plain `for action in actions.iter()` loop. Extending this to also carry headless ingest requests has three obviously-bad shapes and one viable one:

- **(rejected) extend the `StateAction` enum with a `HeadlessIngest` variant.** The `StateAction` type is produced exclusively by `HotkeyStateMachine::drive` (`hotkey/state.rs`), which is **out of boundary** per the §3 "Explicitly NOT touched" list. Adding a variant the FSM has to know how to construct would couple the FSM to a concern outside its responsibility AND require an out-of-boundary edit. Veto.
- **(rejected) recv-and-select inside `run`.** `std::sync::mpsc` cannot be selected on; only `crossbeam-channel`, `tokio::sync::mpsc`, or `flume` offer that. Adopting tokio for one thread is overkill; flume is unfamiliar; `crossbeam-channel` is already in the dependency closure transitively and is the idiomatic Rust answer to "multi-recv select".
- **(rejected) convert the `Receiver<StateAction>` channel to `crossbeam-channel`.** The channel itself is constructed inside `StateDriver::start` in `hotkey/driver.rs` (line 92), which is **also out of boundary** per the §3 "Explicitly NOT touched" list. Changing its type would cascade into the driver, even though no logic changes. Veto.
- **(adopted) sibling crossbeam-channel + in-orchestrator-thread bridge.** Add a second input channel — `crossbeam_channel::{Sender, Receiver}<HeadlessIngestRequest>` — owned by the dictation runtime. Inside the dictation thread, spawn a tiny bridge that does `actions.recv()` on the existing std-mpsc and forwards via `crossbeam_action_tx.send()` to a crossbeam mirror of the action stream. The orchestrator's `run` now takes two crossbeam receivers and `select!`s between them. Total cost: ~30 lines in `runtime.rs` (one bridge thread + one channel pair) plus a `select!` reshape in `dictation.rs::run`. No edits to `hotkey/state.rs`, `hotkey/driver.rs`, or any other §3-listed boundary file.

This topology change is **authorized as in-boundary for Wave 1.1** because it is the minimum necessary to satisfy §3's "headless entry point" contract without disturbing the hotkey FSM or its driver. The `StateAction` semantics, the `HotkeyStateMachine`, the `StateDriver`, and the `Receiver<StateAction>` produced by `StateDriver::start` are all unchanged. The bridge is purely a type adapter inside `dictation::runtime` so the orchestrator can `select!` across two heterogeneous channels.

**Constraints on the topology change:**

- The new channel is mpmc-capable on the producer side (multiple senders: Iter 1's `dictation_import_file` IPC, Iter 3's vault inbox watcher; one consumer: the orchestrator). `crossbeam_channel::Sender` is `Clone`, which satisfies this without further ceremony.
- A `HeadlessIngestRequest` carries `IngestProvenance` (§3) + `Vec<i16>` PCM samples (decoded before send; decoding via `audio::decode::decode_to_pcm16_mono_16k` happens on the producer side so the orchestrator thread is never blocked on file I/O or symphonia codec init).
- Each request includes a per-request reply channel — `crossbeam_channel::Sender<AppResult<i64>>` plus its bounded(1) receiver held by the caller — so the IPC handler can await the new `sessions.id` result synchronously and surface a meaningful toast ("session #123 imported" vs "decode failed: ...").
- The orchestrator processes a `HeadlessIngestRequest` by calling `dictation::ingest::headless_ingest` with its own existing VAD/STT/Cleaner deps (the ones already constructed in `run_dictation_thread`), then sends the result back via the per-request reply channel. The `IngestDeps` struct (§3) is rebuilt for each request from `&mut`/`&dyn` views of the orchestrator's owned deps — zero clones, zero allocations.
- The new dep is `crossbeam-channel` declared in the workspace `Cargo.toml` and re-exported into `src-tauri/Cargo.toml`. The crate is already in the dependency closure transitively (see `Cargo.lock`); promoting it to a direct dep is a workspace-level housekeeping change. Pinned via the workspace's standard version-only style.
- All other §3 boundaries remain intact: no edits to `start_capture`, `discard`, `persist_failed_*`, `pub mod pipeline`, `hotkey/state.rs`, `hotkey/driver.rs`, `hotkey/windows.rs`, the injection layer, the secure-input guard, the recording window, or any of `meetings/*` or `activity/*`.

The `sealed-phases-untouched` judge (Wave 5.3, scheduled for end of Iter 1) will diff-check this boundary mechanically: the only sealed-Phase-3 file edits permitted are inside `src-tauri/src/dictation.rs` (the `run` reshape + `complete()` body refactor authorized in §3) and inside `src-tauri/src/dictation/runtime.rs` (the bridge thread + sender plumb), plus the new files under `src-tauri/src/dictation/` introduced by §3 / §3.1 / this subsection.
