...
- **Iteration 1 beads:**
  ...
  - `mb-7vyz` BLOCKED — Phase D dep-injection architecture decision needed. The dispatch's preferred pattern (extend `StateAction::HeadlessIngest`) requires editing `src-tauri/src/hotkey/state.rs`, which is the dispatch's explicit stop-condition #3 (outside ADR §3 boundary). Deeper: even with the enum variant, the orchestrator's `Receiver<StateAction>` is fed by `StateDriver` (which only takes `HotkeyEvent`), so pattern (a) also needs a sibling mpsc channel + run-loop multi-select. Three viable forks: (b) IPC handler constructs fresh VAD/STT/Cleaner per import (in-boundary, costs one whisper-rs CUDA reload per import call), (a-extended) draft an ADR 0046 amendment for the channel topology change, or defer Phase D entirely. Surfaced to parent; awaiting decision.
  - Then `mb-thmd` (sealed-phases-untouched judge) + `mb-jbf7` (Iter 1 smoke) — both still open, neither dispatched yet.
