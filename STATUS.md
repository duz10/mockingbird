...
- **Status:** Accepted 2026-05-27 (`docs/adr/0046-mobile-extension-via-vault.md`). **Iteration 1 SEALED** (desktop file-ingest pipeline). **Iteration 2 SEALED 2026-05-27** (outbound Obsidian projection live end-to-end, desktop + iPhone verified). **Iteration 3 READY** (inbound mobile→desktop via iOS Shortcut + `<vault>/inbox/` watcher). **Iteration 4 READY** (polish + epic seal) — follows Iter 3.
...
- **Iteration 3: IN-FLIGHT (Wave 0 COMPLETE 2026-05-27; Waves 3.1-3.3 IMPLEMENTATION COMPLETE 2026-05-27; Waves 3.4-3.5 PENDING).** Inbound `<vault>/inbox/` → headless ingest. **Wave 0 spike CLOSED:** 5 observation rounds run by Dustin 2026-05-24 (rounds 1 small text, 2 medium markdown, 3 large text, 4 binary attachment via Obsidian, **4b voice-memo via Files app into `inbox/`** — the spontaneously-added round that mattered most). 8 findings captured at [`docs/spikes/iter3-sync-layer-findings.md`](docs/spikes/iter3-sync-layer-findings.md), all grounded in JSONL log timestamps. Logs at `docs/spikes/iter3-logs/round-{1,2,3,4,4b}*.log`. **CRITICAL Finding 5:** external Files-app writes into Obsidian's iOS sandbox have a **5–15 minute lag** before Obsidian Mobile notices them, unless the app is foregrounded — without mitigation the Shortcut courier would have multi-minute end-to-end latency. **Mitigation shipped:** `docs/mobile/ios-shortcut.md` amended to require a third Shortcut action (`Open App: Obsidian`) as the final step; expected latency drops from minutes to ~30–60s. `mb-s8s2` closed; `mb-mus5` remains closed (amendment is a follow-up doc edit, not re-execution). **Wave 3 implementation beads chartered with findings baked into descriptions:** `mb-9lgi` (Wave 3.1 inbox watcher) **CLOSED in `a3c5900`** ← `mb-txmy` (Wave 3.2 courier processor) **CLOSED in `6b0dd40`** ← `mb-3ivf` (Wave 3.3 InboxRuntime wiring) **CLOSED in `eea2484`** ← `mb-ksau` (Wave 3.4 Iter 3 sealed-phases-untouched judge — PENDING, next dispatch) ← `mb-1yxp` (Wave 3.5 Iter 3 live-fire smoke — PENDING, Dustin's hands with iOS Shortcut). **Live-fire validation on the implementation slate is already green:** the runtime's initial-scan + watcher + courier picked up a pre-existing `New Recording 38.m4a` voice memo on first launch, ran it through whisper-rs CUDA (1079ms STT) + Ollama cleanup (9866ms) into `sessions` row `session_id=116` with `source='mobile-inbox'`, and atomically archived the source to `<vault>/inbox/_archive/2026-05-24/`. This proves the pipeline is sound; `mb-1yxp` separately exercises the full iOS-Shortcut → Obsidian Sync → inbox/ path which Mockingbird itself can't reach. `mb-qxrm` remains a separate Iter 4 Wave 5 hardening matrix (conflict-file injection / machine-fingerprint mismatch / retention nightly / oversized silent-skip / app-offline catch-up), distinct from `mb-1yxp`'s narrower Iter-3-only happy-path smoke.
- **Iteration 4: READY** after Iter 3. Lifts the preview section into a full Mobile Sync tab, wires the remaining 4 settings keys (`VaultSyncBackend` / `SyncTierByteCap` / `VaultDebugKeepCouriers` / `KeepAudioBlobs`), connection health card, nested-vault wizard (`mb-3xww`), `+ Audio file` progress indicator (`mb-q1xt`), final 4 invariant judges, ADR Accepted (final), STATUS update. **No new `phase-*-complete` tag** (lateral epic per LESSONS PINNED P5).
