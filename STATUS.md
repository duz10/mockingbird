...
**KG Phase 1C in flight** — 6-wave ADR-chartered lateral epic (`mb-j368`) under ADR 0051 (Proposed 2026-05-30). Wave 1C.0 (charter) + Wave 1C.1 (Settings tab + activation + boot-vs-poll) + Wave 1C.2 (failed-filings UX) shipped. Wave 1C.5's principal invariant pinned for seal: **`kg-graph-off-ui-untouched`** (with `KgGraphEnabled=false`, every KG UI element except the Settings tab itself stays hidden; no KG IPC invoked from UI — Wave 1C.2 inherits this: the entire "Filing status" card is gated on `kgGraphEnabled === true`). Remaining waves: 1C.3 Dictations filter chips + per-row entity/tag display (`mb-5ly5`); 1C.4 concept modal (`mb-sx6p`); 1C.5 3-judge deterministic gate + ADR Accepted + epic seal (`mb-f4gn`).
