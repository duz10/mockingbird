...
- **Status:** Accepted 2026-05-27 (`docs/adr/0046-mobile-extension-via-vault.md`). **Iteration 1 COMPLETE** (desktop file-ingest pipeline shipped, no Obsidian touchpoints yet). Iteration 2 (outbound Obsidian projection) is the next iteration to start when Dustin says go.
...
- **Iteration 2: READY** (outbound Obsidian projection — Mockingbird-as-source-of-truth, vault-as-projection). Unblocked beads when Dustin says go: `mb-yheh` (vault layout + manifest), `mb-p7cp` (markdown projection), `mb-lvzw` (export reconciliation job). `mb-s8s2` (sync-layer spike) is re-anchored to Iter 2 setup.
