...
- **Iteration 1 beads:**
  ...
  - `mb-evn3` closed (headless ingest extraction + SessionsEventBus at `fcf8008`). `dictation::ingest::headless_ingest(deps, samples, provenance)` is the new pure-Rust entry point; `dictation::events::SessionsEventBus` trait extracted with a `RecordingWindow` blanket impl. `dictation.rs` edits within ADR §3 / §3.1 boundary (free-fn `resolve_active_mode_from_db`; three `emit_session_saved` sites routed through a new helper). Also fixed pre-existing `tests/db_repos.rs` source-field debt.
  - `mb-7vyz` closed (Phase D + ADR §3.2 amendment at `2a4ea12`). Iter 1 implementation slate **complete (4/4)**. ADR §3.2 now authorizes the orchestrator's two-channel topology: a sibling `crossbeam-channel` carries `HeadlessIngestRequest`s alongside the existing `StateAction` stream, with an in-thread mpsc→crossbeam bridge so the still-untouched `StateDriver` channel can be `select!`ed. `+ Audio file` button live on the Dictations page; `dictation_import_file` IPC decodes off-thread, queues the request, replies via per-request bounded(1) channel. No fresh VAD/STT/Cleaner allocations per import.
  - `mb-thmd` closed 2026-05-27 (sealed-phases-untouched judge 🟢 PASS @ 95% conf). Verdict: `docs/judges/mobile-extension/sealed-phases-untouched-iter1-verdict.md`. Diff `d99a4cd..a004efa` stayed inside ADR §3 / §3.1 / §3.2 boundary; all forbidden subsystems (`hotkey`/`meetings`/`activity`/`injection`/`window_context`/`secrets`/`stt`/`cleanup`/`recording_window.rs`) show zero diff; migrations 001-017 modification-free; no `UPDATE` against `stage='raw'`. One strict-reading note: the §3.1 single-emit-point routing landed at all three persist sites (`persist_complete` + the two failure paths), which is the literal realization of §3.1's "converge on a single emit point" — ADR text supersedes kickoff-prompt's stricter paraphrase per `AGENTS.md`. 5% confidence gap is the residual that this reading is wrong; remedial action would be a 2-line revert in the two failure paths if so.
  - `mb-jbf7` Iter 1 smoke (OPEN). 2026-05-23 status: release binary rebuilt (5m 43s, `target/release/mockingbird.exe` mtime 13:00:31, 51 MB). Strategy-A programmatic smoke (a `src-tauri/examples/smoke_iter1_ingest.rs` that drives `DictationRuntime` + `headless_ingest()`) is blocked by `mb-0n8c`: the example binary builds clean (4m 30s, 41 MB, link-clean) but exits `STATUS_ENTRYPOINT_NOT_FOUND` (0xc0000139) identically to `cargo test --release` (LESSONS PINNED P2; refined today). Fell back to Strategy B: launched via `scripts\run-mockingbird.ps1`, process PID 70808 alive past warmup, **zero WARN/ERROR/panic** in startup log chunk (Whisper-CUDA loaded GPU, Ollama warmup 9.3s). Migration 018 verified applied via `verify_iter1_schema.py` (pure-rusqlite read-only probe): `schema_version` bumped 17→18, `sessions.source` column (TEXT NOT NULL DEFAULT 'desktop') present at cid=21, `idx_sessions_source` index present, all 111 legacy rows backfilled with `source='desktop'`. **Pass-criterion mapping:** criterion 4 (schema half) PASS; criteria 1-3 (decode + transcribe + clean), 5 (UI refetch), 6 (PTT regression), 7 (second memo) all need Dustin's hands-on click-test against the fixture `C:\Users\dboyd\Downloads\New Recording 38.m4a` via the new `+ Audio file` button on the Dictations page. Smoke example + Cargo.toml entry + Python probe committed in-tree so the day `mb-0n8c` lands, the smoke is ready. LESSONS body entry `2026-05-23 [ADR 0046 Iter 1 / mb-jbf7]` documents the strategy-A blockage and the pair-with-rusqlite-probe pattern for future ingest iterations.
