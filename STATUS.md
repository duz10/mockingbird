...
**Last consolidated:** 2026-05-30 (KG Phase 1C Wave 1C.2 shipped — failed-filings UX + queue-status line + idempotent retry; mb-9ufg closed; ADR 0051 still Proposed, seals after 1C.5). Prior anchors: 2026-05-30 (Wave 1C.1 shipped — Settings KG tab + activation toggle + boot-vs-poll worker promotion); 2026-06-03 (ADR 0050 KG Phase 1B SEALED); 2026-05-30 (Wave 1C.0 charter).
...
**KG Phase 1C in flight** — 6-wave ADR-chartered lateral epic (`mb-j368`) under ADR 0051 (Proposed 2026-05-30). Wave 1C.0 (charter) + Wave 1C.1 (Settings tab + activation + boot-vs-poll) + Wave 1C.2 (failed-filings UX) + Wave 1C.3 (Dictations retrieval UX) shipped. Wave 1C.5's principal invariant pinned for seal: **`kg-graph-off-ui-untouched`** (with `KgGraphEnabled=false`, every KG UI element except the Settings tab itself stays hidden; no KG IPC invoked from UI — Wave 1C.3 inherits this: the entire FilterBar + per-row chip strip is gated on `kgGraphEnabled === true`). Remaining waves: 1C.4 concept modal (`mb-sx6p`); 1C.5 3-judge deterministic gate + ADR Accepted + epic seal (`mb-f4gn`).
