...
- **Iteration 3: IN-FLIGHT (Wave 0 infrastructure shipped 2026-05-27).** Inbound `<vault>/inbox/` → headless ingest. Wave 0 spike infrastructure now landed: `scripts\watch-vault.ps1` (PS 5.1 FileSystemWatcher → JSONL log, smoke-tested green) + `docs/spikes/iter3-sync-layer-observation-plan.md` (6-round observation plan + analysis question list). Wave 3.3 iOS Shortcut spec + user setup doc shipped at `docs/mobile/ios-shortcut.md` per ADR §8 (Variant 1 = Ask Where to Save ON, sidecar descoped per `mb-0uqb`); `mb-mus5` CLOSED. `mb-s8s2` remains in_progress with handoff prepped for Dustin's hands-on observation rounds (coordinated by planning-agent in a follow-up dispatch). Implementation beads (Wave 3 watcher + courier pickup, charter blocked on observation findings via `mb-9lgi`) not yet authored — that's the follow-up dispatch's job.
