...
**Phase 10 — Activity Capture (sibling subsystem). Wave 4 — Audio Layer (Layer 2) code-complete. Awaiting Dustin direction on Wave 5 ADR 0038 (encryption-at-rest) before further dispatch.**
...
- **Wave beads** (each P1, dependency-chained so each subsequent wave depends on its predecessor; the epic depends on all of them):
  ...
  - `mb-g1w2` **Wave 4: Audio layer (Layer 2)** ← `in_progress` (code complete this iteration; pure-Rust `activity/audio.rs` `AudioPipeline` trait + `LongFormAudioPipeline` impl wrapping Meeting Capture's WASAPI twin-stream + `LongFormStt` chunked Whisper; per-session transcripts persisted in `activity_transcript_segments` via `activity/segments_persist.rs`; `activity/block_audio_stitcher.rs` midpoint-rule attribution of segments to Blocks; `activity/abstractor.rs` extended to select the `abstract_block.audio_aware.md` template + `abstract_v2_audio-XXXXXXXX` prompt-version family when audio is present, honoring `user_edited`; migration 014 adds `audio_whisper_model` + `audio_chunk_window_ms` columns to `activity_sessions` for provenance; `activity_start(with_audio: bool?)` IPC + Command Center default-from-setting wiring; Settings General row + `activity_audio_enabled` typed setting (default OFF — privacy by default); ADR 0041 Accepted inline; 13 pure-module stitcher tests via throwaway crate all green; awaiting Dustin direction on Wave 5 ADR 0038 — Bernard stops here per kickoff)
  - `mb-a6tz` Wave 5: Hardening and polish (charters ADR 0038, encryption-at-rest) — **next; blocked on Dustin's say-so since encryption is security-sensitive**
