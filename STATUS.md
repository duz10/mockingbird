...
- **Iteration 3: READY** (when Dustin says go). Inbound `<vault>/inbox/` → headless ingest. Owns Wave 0 sync-layer spike (`mb-s8s2`, observable findings inform combined-detector params), Wave 3 watcher + courier pickup (`mb-mus5` iOS Shortcut spec + setup docs).
