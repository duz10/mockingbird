...
**Nothing in flight.** KG Phase 1C sealed (ADR 0051 Accepted 2026-05-31); Phase 1D (one-shot backfill of pre-Phase-1 entries) awaits Dustin kickoff.

### Phase 1D kickoff notes (from the Phase 1C seal)

- **All 6 retrieval axes online except category.** The 5 axes that
  shipped at 1C.3/1C.4 (entity filter, tag filter, free-text query,
  per-row chip strip, filing-state pills, concept modal entity + tag
  drill-down) cover everything but category. The category-axis gap
  is `mb-oji5`, blocked on Phase 1D — verified twice that Phase 1B
  schema has no queryable `category` column (the classify pass
  produces `Entry.category` but it's in-memory only), and ADR 0051
  §"Out of scope" explicitly bans new migrations in 1C. Phase 1D's
  first migration should add `sessions.category TEXT NULL` (or an
  equivalent join table); the UI side already has the typed wire
  shape via `SearchFilter` (additive).
- **Latency budget empirically locked at p95=59s per filing**
  (`docs/knowledge-graph/phase-1c-latency-baseline.md`); the
  `extract` + `extract_entities` passes together = ~82% of pipeline
  cost. A 500-entry backfill at current throughput is ~8 hours
  wall-clock. Plan accordingly.
- **`KgGraphEnabled` is now load-bearing for backfill gating.** The
  graph-off invariant judge (Rust-side `kg_graph_off_invariant` from
  Phase 1B + UI-side `ui/tests/kg-graph-off-invariant.spec.ts` from
  Phase 1C.5) is the contract: with the toggle off, no `kg_*` row
  written, no `kg_*` IPC invoked from the UI (except
  `kg_settings_get_all` which is read-only). Backfill must respect
  the same off-by-default contract.
- **Standing beads carrying forward:** `mb-bbl2` (sonner retrofit
  for native browser confirms), `mb-y6pq` (`--status-bad` token
