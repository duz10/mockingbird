...
**Last consolidated:** 2026-05-31 (**KG Phase 1C SEALED via ADR 0051 Accepted** — Wave 1C.5 shipped graph-off-UI invariant judge + a11y polish + epic seal; mb-j368 + mb-f4gn closed; mb-oji5 deferred to Phase 1D with explicit blocker). Prior anchors: 2026-05-30 (Wave 1C.3 shipped — Dictations retrieval UX); 2026-05-30 (Wave 1C.2 shipped — failed-filings UX + queue-status line + idempotent retry); 2026-05-30 (Wave 1C.1 shipped — Settings KG tab + activation toggle + boot-vs-poll worker promotion); 2026-06-03 (ADR 0050 KG Phase 1B SEALED); 2026-05-30 (Wave 1C.0 charter).
...
**KG Phase 1D — source-gated filing + first-class KG screen** (ADR 0052 Proposed 2026-06-04). Re-scopes the standing "Phase 1D = backfill" framing (now moot per the 2026-06-04 source-of-truth alignment review against the original product spec §15 + Clark article). Six waves; Waves 1D.0, 1D.1, 1D.2, 1D.3, 1D.4, **and 1D.5 (Settings expansion + launch-into-Obsidian) complete**. Wave 1D.6 (judges + seal) is the only remaining item.

- **Charter ADR:** `docs/adr/0052-knowledge-graph-phase-1d-charter.md` (Proposed; flips Accepted at Wave 1D.6 seal).
- **Phase doc:** `docs/phases/phase-1d.md` (full six-wave plan).
- **Bead epic:** `mb-x7f9`; sub-beads `mb-qhll` (1D.0), `mb-pxzk` (1D.1), `mb-j00j` (1D.2), `mb-0gt6` (1D.3), `mb-6hm2` (1D.4), `mb-navi` (1D.5), `mb-q2p1` (1D.6).
- **Two drifts being corrected:** (1) ADR 0050's dictation-tail hook auto-files every dictation when toggle on — should fire only for dictations with `source = 'kg-note'`; standard Right Alt PTT + `+ New Dictation` must never feed the KG. (2) Per spec §15.3 the KG should be a first-class left-sidebar destination with read-only dashboard + audio note + text note + launch-into-Obsidian; Phase 1C narrowed "new UI" to filter chips on Dictations which was correct for the toggle but wrong as the v1 home. Wave 1D.4 relocates the chips to the new KG screen.
- **Re-numbering:** Phase 1D = this charter; Phase 1E = Obsidian-as-source-of-truth (vault projection + reverse-watcher per ADR 0048 Q3 — future ADR 0053); Phase 1F (was 1E) = v1 beta tag.
- **Clean-slate verify (Wave 1D.0):** NOT clean — `kg_entities=3`, `kg_filing_queue=2` (both `done`), `kg_entity_mentions=3`, `kg_tag_mentions=11`, `kg_graph_enabled='true'` against `sessions.id` 128 + 129 (2026-05-30 dev test toggles). Wave 1D.1 migration 025 purges all four KG mention/queue/entity tables + resets the toggle in the same transaction as the schema change. **Live-applied 2026-06-04** against Dustin's actual `%APPDATA%\com.dustin.mockingbird\mockingbird.db`: pre = (3 entities / 3 entity-mentions / 11 tag-mentions / 2 filing-queue / toggle=true / schema 24), post = (0 / 0 / 0 / 0 / toggle=false / schema 25), 129 sessions all backfilled to `capture_kind='dictation'`. Backup written to `backup-pre-mb-pxzk/` alongside.
- **Wave 1D.1 result (mb-pxzk closed 2026-06-04):** Migration 025 ships two new sessions columns (`capture_kind TEXT NOT NULL DEFAULT 'dictation'`, `category TEXT NULL`) + composite index `idx_sessions_capture_kind(capture_kind, started_at DESC)` + the kg_* drift purge + toggle reset, all in one transaction. **Naming note:** kickoff text proposed naming the new column `source`, which would have collided with the pre-existing `sessions.source` from migration 018 (audio-origin axis: `desktop`/`desktop-import`/`mobile-inbox`). Renamed to `capture_kind` to keep "one column, one meaning" — documented in the migration SQL header + the `CaptureKind` enum docs. Reserved `KgNoteText` variant is structurally present for ADR 0052 §D3 text-notes even though those bypass the sessions table entirely; the dictation-tail gate rejects it defensively. **Three-gate cascade in `try_enqueue_for_kg_filing`:** outcome → source (NEW) → toggle. `kg_graph_off_invariant` probe extended to sweep 8 InjectionOutcome variants × (Dictation, KgNote) = 16 sub-checks + source-gate negative control + positive control — all green. `kg_parity 32/32` green in both default + `--persist` modes.
- **`mb-oji5` (category-axis persistence)** — closed at 1D.0, consumed by Wave 1D.1's `sessions.category` column.
- **Latency budget:** carries forward from 1C at **p95=59s per filing** (qwen2.5:7b; `extract`+`extract_entities` = ~82% of pipeline cost). Phase 1D does not change pipeline shape; re-measurement only if 1D.3 text-note ingest shifts the cost shape.
- **Wave 1D.5 result (mb-navi closed 2026-06-04):** Settings -> KG tab fleshed back out post-1D.4 with four read-only reference cards + one side-effect button: (1) **Vault** card — read-only mirror of `vaultPath` from the ADR 0046 Mobile Sync settings (single source of truth — no duplicate picker here); when unset, inline link cross-navigates to the Mobile Sync tab via a new `onOpenMobileSync` callback prop the parent (`Settings.tsx`) supplies by calling its existing `setTab('mobileSync')`. (2) **Vocabularies** card — read-only display of the v1 controlled lists (Layer 1 categories: personal/professional/objective; Layer 2 entry types: task/research/idea/note/reference), pulled from a new static IPC `kg_vocabularies_get()` whose payload is statically derived from the `kg::schema::{Category, EntryType}` enums and pinned by a `vocabularies_matches_schema_enums` unit test that asserts both lists match the enum variants byte-for-byte (cross-language drift becomes a compile/test failure). (3) **Processing-mode** card — one-line "Ingest mode: silent" indicator per spec §15.5 plus body copy. (4) **Dual-write** card — one-line reminder that KG audio notes also land in Dictations history but text notes are KG-only. **Launch-into-Obsidian button**: same button shipped on both the Settings KG tab AND the KG dashboard's new `ActionsBand` (`ui/src/routes/knowledge-graph/Actions.tsx`, ~90 LoC). Disabled + tooltipped when vault unconfigured. Click invokes `kg_launch_obsidian()` (new IPC) which reads `SettingKey::VaultPath`, then shells out to `cmd /c start "" obsidian://open?vault=<encoded-leaf-name>` via new pure-Rust module `src-tauri/src/kg/launcher.rs` (~210 LoC; Windows arm + macOS/Linux stub error arms per Principle 5). Launcher includes a 10-line minimal RFC-3986 percent-encoder for the vault leaf-name with unit tests pinning space → `%20` (NOT `+`), `&`/`=`/`?`/`#`/`+`/`%` percent-encoding, and UTF-8 multibyte handling — added rather than pulling the `url` crate dependency for one call site. Graph-off invariant `OFF_MODE_ALLOWLIST` extended from `{kg_settings_get_all}` to `{kg_settings_get_all, kg_vocabularies_get}` with an explicit comment that the contract is "no graph DATA touched when off", not the literal "no kg_* IPC at all" — vocabularies are static enum-derived metadata. **No tauri capabilities change needed** (the existing `core:default` permission covers `#[tauri::command]` invocations). **Cargo gate green** (fmt/clippy `--release -D warnings`/test --release --no-run: 9m24s, 17 test binaries linked clean per LESSONS P2 fallback). **Live launcher unit tests green** via throwaway-crate recipe (5/5 — `encode_vault_name` ×4 + `launch_errors_when_path_has_no_leaf` ×1). **UI gate green** (tsc clean / vitest 140 passed / vite build). Files touched: `src-tauri/src/kg/{launcher.rs (NEW), mod.rs}`, `src-tauri/src/commands/{kg.rs, mod.rs}`, `ui/src/lib/{tauri.ts, types.ts}`, `ui/src/pages/{Settings.tsx, SettingsKgTab.tsx}`, `ui/src/routes/knowledge-graph/{Actions.tsx (NEW), Dashboard.tsx}`, `ui/src/i18n/en.json` (+~18 keys), `ui/tests/kg-graph-off-invariant.spec.ts`. Out-of-scope deferrals: **vault subtree creation** (`<vault>/Knowledge Graph/{Inbox,Entries,History}/`) and reverse-watcher remain Phase 1E Wave 1E.1 scope; **vocabularies EDITOR** deferred to post-v1 (filed as `mb-0ui1` P3 — "KG: editable vocabularies in Settings").
- **Wave 1D.4 result (mb-6hm2 closed 2026-06-04):** Relocated the KG retrieval surface from Dictations to the new `/knowledge-graph` route per ADR 0052 §D5. **Dictations.tsx is now KG-free** (-211 LoC; chip filter row, per-row chip strip, concept-modal entry points, kg_entries_summary + kg_search_entries plumbing all gone — Dictations is back to its pre-1C shape: history list + FTS5 search + detail pane). **New `ui/src/routes/knowledge-graph/Retrieval.tsx`** band on the dashboard (~280 LoC) hosts the filter chips + filter-driven row list; mounts between QueueBand and RecentActivityBand. **Dashboard.tsx** now owns the single ConceptModal instance; both Retrieval rows AND RecentActivityBand rows surface chip clicks via `onConceptOpen` (1D.2-shipped click-to-noop chips are now click-to-modal — closes the kickoff acceptance gate #3). **FlaggedBand** gained click-to-retry: `kg_requeue_failed(queueId)` + post-success refetch via the same `refetch()` callback CaptureBand already uses; idempotent at the Rust layer (Wave 1C.5 J3 invariant unchanged) and disable-on-click in the UI to keep one round-trip per click. **`SettingsKgFailedFilings.tsx` deleted** — Settings -> KG tab is now toggle-only (plus 1D.5's pending additions). **Category-badge decision (ADR 0052 §D6 follow-up):** removed from Dictations rows entirely (option (a) from the kickoff). Rationale: `sessions.category` is populated ONLY by the KG filing worker for `capture_kind='kg-note'` rows, so the badge would be NULL on 100% of `capture_kind='dictation'` rows — i.e. dead code on the Dictations page by definition. Cleaner separation of surfaces; matches the source-gating philosophy from 1D.1 (one column, one meaning, one home). Category lives on the KG screen where its source-gating semantic is the default expectation. **Tightened graph-off-UI invariant judge (J3 strengthening per kickoff acceptance gate #4):** `kg-graph-off-invariant.spec.ts` now adds Walk 4b — with toggle ON, navigate to /dictations, click around (row select + search input fill), snapshot the `__KG_IPC_SPY__` calls before and after, assert the diff is empty. Proves Dictations never fires a `kg_*` IPC regardless of toggle state. Existing Walk 4 still fires `kg_dashboard_snapshot` when /knowledge-graph mounts (positive control intact). Spec files `kg-failed-filings.spec.ts` → `kg-flagged-band.spec.ts` and `kg-dictations-retrieval.spec.ts` → `kg-retrieval.spec.ts` renamed + rewritten against the new route shape; `kg-concept-modal.spec.ts` rehomed on the KG dashboard. **UI gate green** (tsc/vitest 140 passed/npm build); **Cargo gate green** (fmt/check; no Rust changes this wave). Files touched: `ui/src/pages/{Dictations,DictationsList,DictationsDetailPane,SettingsKgTab}.tsx` (all subtractive; -~400 LoC combined), `ui/src/pages/SettingsKgFailedFilings.tsx` (deleted), `ui/src/routes/knowledge-graph/{Retrieval.tsx,Retrieval.module.css}` (new), `ui/src/routes/knowledge-graph/{Dashboard.tsx,Dashboard.module.css}` (extended), `ui/src/i18n/en.json` (+4 keys), `ui/tests/{kg-flagged-band,kg-retrieval,kg-concept-modal,kg-graph-off-invariant}.spec.ts` (rewrites + rename). **Out-of-scope deferral:** row-click on Retrieval band is visual-only this wave (no detail pane to navigate to); deep-linking to `/dictations#session=<id>` filed as a follow-up bead `mb-2wbk` per ADR 0052 §"Out of scope".
- **Wave 1D.3 result (mb-0gt6 closed 2026-06-04):** KG dashboard now carries a `CaptureBand` with two lanes. (1) **Audio note** lane: programmatic `dictation_start_kg_note` IPC injects synthetic KeyDown with a new sibling `next_start_is_kg_note: Arc<AtomicBool>` flag (independent axis from `next_start_is_programmatic` so plain in-app dictation stays `capture_kind='dictation'`); orchestrator swap-and-pins `CaptureKind::KgNote` at `start_capture`; row dual-writes into Dictations history AND fires KG enqueue via the 1D.1 source-gate. Stop reuses the existing `dictation_stop` IPC. Lane subscribes to `history:session-saved` to flip status to "Filed" without polling. (2) **Text note** lane: new `kg_ingest_text_note(text)` IPC routes through `kg::ingest_text::ingest_text_note` (new 421-line module, sibling of `dictation::ingest`) — bypasses Whisper entirely, the typed string IS raw/cleaned/final transcript; row lands with `capture_kind='kg-note-text'`. `commands/sessions.rs::list_sessions` now filters `WHERE capture_kind != 'kg-note-text'` so text notes never surface on the Dictations history page (acceptance gate #2). KG outcome cascade re-used unchanged: graph-on → enqueue; graph-off → row exists, KG queue stays empty (Phase 1E backfill picks it up if user later flips toggle). **Refactor seam discovery:** phase doc anticipated extracting `kg::pipeline::run_from_text` as a shared entry point; inspection showed `run_pipeline` already takes `&str`, so the audio + text paths converge naturally at the `transcripts.text` row — no shared abstraction needed, saved ~100 LoC of unnecessary plumbing (logged in LESSONS). `resolve_active_mode_from_db` factored into `resolve_active_mode_from_conn` so text-note ingest can reuse mode resolution without re-locking. Dictation orchestrator integration test extended to pass the new flag (always `false` in PTT fixtures). Graph-off-UI invariant judge unchanged — CaptureBand is mounted by `KnowledgeGraphDashboard` which is gated by the route guard on `kgGraphEnabled === true`, so OFF-mode users never see (or fire IPCs from) the capture lanes. Cargo gate (fmt/clippy/test-no-run all green; 17 release-test binaries linked clean per LESSONS P2 fallback) + UI gate (tsc/vitest 140 passed/npm build) all green. Touched files: `commands/{dictation,kg,mod,sessions}.rs`, `db/sessions.rs`, `dictation.rs`, `dictation/runtime.rs`, `kg/mod.rs`, `kg/ingest_text.rs` (NEW), `lib.rs`, `tests/dictation_orchestrator.rs`, `ui/src/routes/knowledge-graph/{CaptureBand.tsx,CaptureBand.module.css}` (NEW), `ui/src/routes/knowledge-graph/Dashboard.tsx`, `ui/src/i18n/en.json`, `ui/src/lib/tauri.ts`. **Discipline note:** a prior dispatch was API-error-orphaned mid-way through — Bernard recovered by inventorying uncommitted disk state vs. the Wave 1D.2 seal (`37feed7`), confirming no duplication, finishing the missing UI piece + the `list_sessions` filter + the IPC handler wiring, then sealing. LESSONS pattern: when the kickoff explicitly warns of orphaned work, audit `git diff --stat` against the last seal commit BEFORE writing anything new.
- **Wave 1D.2 result (mb-j00j closed 2026-06-04):** New top-level route `/knowledge-graph` + sidebar entry (conditional on `KgGraphEnabled`, reactive to toggle via zustand store). Read-only dashboard with 5 bands: Counts (entity totals + entities-by-type breakdown + filed-entry totals), Queue state (per-state line + last-success time), Recent activity (last 10 done filings with chip strips re-using `DictationKgChips` inert mode), Flagged for review (==`state='failed'` per phase doc; v1F may add confidence-based flags), Upcoming due dates (empty-state placeholder — phase 1E populates once `due_iso` is persisted). Single new IPC `kg_dashboard_snapshot()` (one round-trip; honors graph-off contract by returning an empty snapshot without DB reads). `QueueStatus` extended additively with `done: u32`. Graph-off-UI invariant judge (`ui/tests/kg-graph-off-invariant.spec.ts`) extended with 1D.2 walks 3b, 3c, 4b: sidebar item hidden when OFF, direct-URL `/knowledge-graph` renders disabled-state without firing `kg_dashboard_snapshot`, positive control after toggle ON fires the dashboard IPC. Cargo gate (fmt/clippy/test-no-run) + UI gate (tsc/vitest 140 passed/npm build) all green.
- **Standing beads carrying forward:** `mb-bbl2` (sonner retrofit for native browser confirms), `mb-y6pq` (`--status-bad` token sweep), `mb-26aw` (`smoke.spec.ts` ×4 pre-1C Playwright failures).
