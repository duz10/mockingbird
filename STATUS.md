...
**Last consolidated:** 2026-05-30 (KG Phase 1C Wave 1C.4 shipped — concept modal + 2 detail IPCs + chip-strip click-to-open wiring; mb-sx6p closed; ADR 0051 still Proposed, seals after 1C.5). Prior anchors: 2026-05-30 (Wave 1C.3 shipped — Dictations retrieval UX); 2026-05-30 (Wave 1C.2 shipped — failed-filings UX + queue-status line + idempotent retry); 2026-05-30 (Wave 1C.1 shipped — Settings KG tab + activation toggle + boot-vs-poll worker promotion); 2026-06-03 (ADR 0050 KG Phase 1B SEALED); 2026-05-30 (Wave 1C.0 charter).
...
**KG Phase 1C in flight** — 6-wave ADR-chartered lateral epic (`mb-j368`) under ADR 0051 (Proposed 2026-05-30). Wave 1C.0 (charter) + Wave 1C.1 (Settings tab + activation + boot-vs-poll) + Wave 1C.2 (failed-filings UX) + Wave 1C.3 (Dictations retrieval UX) + Wave 1C.4 (concept modal + chip click-to-open) shipped. Wave 1C.5's principal invariant pinned for seal: **`kg-graph-off-ui-untouched`** (with `KgGraphEnabled=false`, every KG UI element except the Settings tab itself stays hidden; no KG IPC invoked from UI — Wave 1C.3+1C.4 inherit this: the entire FilterBar + per-row chip strip + ConceptModal mount-point are all gated on `kgGraphEnabled === true`). Remaining waves: 1C.5 3-judge deterministic gate + ADR Accepted + epic seal (`mb-f4gn`).

**Wave 1C.4 SHIPPED (2026-05-30, `mb-sx6p`):** Concept page modal (entity / tag detail view per ADR 0051 §D4 "modal in v1"). Two new typed IPCs: `kg_entity_detail(entityId) -> EntityDetail { entityId, canonicalName, entityType, mentionCount, aliases: string[], recentEntries: EntryRef[], totalEntries }` + `kg_tag_detail(tagId) -> TagDetail { tagId, tag, mentionCount, recentEntries: EntryRef[], totalEntries }`. Both consume the migration-024 concept VIEWs (`kg_concept_entities_view` + `kg_concept_tags_view`) joined with a CTE-built entries-summary view yielding the `EntryRef { entryId, title, capturedIso, category? }` shape (category is always `None` in 1C.4; persistence change tracked as `mb-oji5` for 1C.5+). Store helpers in `kg/store/{entities,tags}.rs` (4 + 4 throwaway-crate tests; 16/16 green incl. empty-entity defensive, alias-roundtrip, recent-limit cap at 50, nonexistent-id → `Err`, newest-first ordering, distinct-entry dedup across multiple mentions). Commands registered in `commands/kg.rs` + `commands/mod.rs::generate_handler!`; capabilities/default.json untouched (wildcard `core:default` per Wave 1C.1 LESSONS). UI: NEW `ui/src/pages/ConceptModal.tsx` (332 LoC) composing the existing `design/components/Dialog` primitive with two modes (`entity` / `tag`); header carries the entity canonical_name + a small `entityType` badge OR the tag rendered as `#slug`; body shows mention_count + aliases section (entities only) + scrollable recent_entries list (cap 50) where each row = title + category badge (when present) + `captured_iso` (formatted via existing `formatRelativeIso`) + click-to-jump; footer renders `Total entries: N` + Close button. Dialog primitive handles focus trap + Escape via portal; structurally verified by an end-to-end read of `Dialog.tsx` per the 1C.3 risk-flag-1 discipline. NEW supporting CSS module `ConceptModal.module.css` (tokens-only). EDIT `ui/src/pages/DictationKgChips.tsx`: chips become `<button>` with new `kg.chip.entityOpenAria` / `kg.chip.tagOpenAria` aria-labels ("Open concept page for entity {name}" / "Open concept page for tag {name}") **whenever** an `onConceptOpen` callback is passed down; click stops row-select propagation. Old `<span>`-with-`Entity {name}`-label path preserved for callers that omit the callback (currently no production caller — Dictations.tsx always passes it when `kgGraphEnabled === true`). EDIT `ui/src/pages/Dictations.tsx`: page-level `activeConcept: { kind: 'entity', entityId } | { kind: 'tag', tagId } | null` state hoisted alongside existing detail-pane plumbing; callback prop forwarded to `DictationsList` → `DictationKgChips`; entry-row click in the modal closes the modal first, then dispatches the existing `setSelectedEntryId` to surface the row in the detail pane (close-then-select per kickoff D5 to avoid eye-track confusion). Dictations.tsx final LoC: **558** (was 549 post-1C.3; +9 for state hoist + callback wiring + modal mount, well under the 600 ceiling). NEW IPC helpers `kgEntityDetail` / `kgTagDetail` in `lib/tauri.ts` with default fixtures (clean-empty shapes) for `npm run preview`. NEW `EntityDetail` + `TagDetail` types in `lib/types.ts`; `EntryRef` reused from 1C.3. 8 new `kg.concept.*` i18n keys + 2 new `kg.chip.*` open-aria keys (en.json). Sonner toast on `kg_entity_detail` / `kg_tag_detail` load failure (errors only; opening a modal is its own success confirmation). Zero hex/rgb/hsl literals; zero `any`. Playwright single-judge sweep `ui/tests/kg-concept-modal.spec.ts` (1 test / 6 assertions; entity drill-down + tag drill-down + click-to-jump + Escape-to-close + no-console-errors) authored by code-puppy per the same qa-kitten fallback as 1C.3. Authored alongside a 2-line update + 9-line comment to `ui/tests/kg-dictations-retrieval.spec.ts` to track the chip-aria-label drift caused by the chip becoming interactive (see LESSONS 2026-05-30 1C.4 Finding 1). Gates: cargo `fmt --check` + `clippy --release -- -D warnings` + `test --release --lib --no-run` (5m45s) all green; UI `tsc --noEmit` + vitest 140/140 + `npm run build` all green; lint skipped per `mb-yxh`; KG Playwright specs 3/3 (kg-concept-modal 1/1, kg-dictations-retrieval 1/1, kg-failed-filings 1/1). Pre-existing Playwright failures (smoke.spec.ts ×4 + kg-settings-tab unscoped `role=status` strict-mode violation) filed as `mb-26aw` (P3, pre-1C.4 baseline, not gating 1C.4). LESSONS body covers 3 findings (interactivity-vs-aria-label regression class; clippy `doc_lazy_continuation` trap on `///` continuation lines starting with `+`/`*`/`-`; end-of-iteration Playwright triage discipline — bisect → bead → carry on). Commits: store helpers `a1a6a07`, commands `296afe8`, ui-author `032b27d`, gate fixes (fmt + clippy doc-continuation) `f30c528`, Playwright sweep + 1C.3 spec drift fix `eb534f4`, STATUS+LESSONS+bd this commit.

**Wave 1C.3 SHIPPED (2026-05-30, `mb-5ly5`):** Dictations retrieval UX surfaced via 4 typed IPC wrappers (camelCase) over the Wave-1C.3-backend triplet `kg_search_entries / kg_list_entities / kg_list_tags / kg_entries_summary` (all DTOs serialize camelCase, verified by unit test on the Rust side). New `DictationsFilterBar.tsx` (397 LoC) hosts an entity multi-select combobox + a tag multi-select combobox with 200ms debounced prefix autocomplete; selected chips dismissable; suggestions popover uses `role="listbox"` / `role="option"` for keyboard + SR navigation; `onMouseDown` on suggestions to beat the input's `onBlur` close-race. New `DictationKgChips.tsx` (182 LoC) per-row strip = top-5 entity chips (server-ordered DESC by mention rank) + "+N more" overflow + same shape for tags + filing-state Pill (pending/processing render “Indexing” variants; failed renders red “Indexing failed” with "See Settings → Knowledge Graph" tooltip; done + not_enqueued render no pill; centralised mapping table sidesteps `noUncheckedIndexedAccess` widening). Chips are visually inert this wave (click-to-add deferred to 1C.4 per kickoff risk list). Filter integration: when toggle ON + any axis set, parent calls `kgSearchEntries(filter)` once (200ms debounced) and intersects with the existing list / FTS-hits pipelines (within-axis OR, across-axis AND, query axis UNION-style with FTS hits per brief D6); ONE batched `kgEntriesSummary(visibleIds)` IPC per visible-id set (per-row firing is the kickoff stop condition); refresh-on-tab-focus via a `summaryNonce` bump (NO polling, NO Tauri events per brief D8). Sonner toast (already mounted in `main.tsx`) carries the filter-load error path. Empty state "No dictations match these filters" + Clear-filters button resets all axes (chips + query). Per kickoff pre-flight finding: **Dictations.tsx came IN at 830 LoC, OUT at 549 LoC** — extracted DictationsLlmPassCard (192 LoC), DictationsList (205 LoC; new mount point for the chip strip + `KgSummaryMap` prop), and DictationsDetailPane (182 LoC; reserved `kgSummary` prop forward-threaded for 1C.4 concept modal). New `SearchFilter` + `EntitySuggestion` + `TagSuggestion` + `EntityRef` + `TagRef` + `FilingState` + `EntrySummary` types in `lib/types.ts`; default fixtures (empty/zero) registered in `lib/tauri.ts` for all 4 new IPCs. 26 new `kg.filter.*` + `kg.chip.*` i18n keys. New CSS modules: `DictationsFilterBar.module.css` (158 LoC), `DictationKgChips.module.css` (98 LoC) — tokens-only (`--mode-normal`, `--status-error`, `--surf-2/3`, `--border`, `--r-pill`, `--r-2/3`, `--s-1/2/3`, `--type-xs/sm`, `--on-surf` family). +11 IPC-contract vitest cases (`DictationsKgRetrieval.test.ts`) covering default-empty + override + arg-shape + numeric-Rust-key-stringified-at-JSON-boundary. Zero hex/rgb/hsl literals, zero `any`. Gates: `npx tsc --noEmit` green, vitest 135/135 (was 124; +11), `npm run build` green; lint skipped per `mb-yxh`. LESSONS body covers 3 findings (CSS-module-class `string | undefined` widening under `noUncheckedIndexedAccess`; three-way Dictations.tsx extraction discipline when over-ceiling pre-wave; JSON-boundary stringification of Rust `HashMap<i64, _>` IPC returns). Commits: this commit.

**Wave 1C.2 SHIPPED (2026-05-30, `mb-9ufg`):** Backend triplet `kg_list_failed_filings(limit) -> Vec<FailedFiling>` (default cap 50, newest-first) + `kg_requeue_failed(queue_id) -> ()` (idempotent on already-pending rows per J3 invariant) + `kg_queue_status() -> QueueStatus { pending, processing, failed, lastDoneIso }` registered in `commands/mod.rs`'s `invoke_handler!`; DTOs serialize camelCase. UI: new `ui/src/pages/SettingsKgFailedFilings.tsx` (340 LoC) hosting the entire "Filing status" Card (status line + failed-filings list + transient toast); `SettingsKgTab.tsx` grows from 163 to 192 LoC (+29) with a single conditional child mount gated on `kgGraphEnabled === true` (so the child's mount/unmount cleanly re-fetches on every off→on flip; refresh strategy is on-mount + on-retry-success, NO polling, NO Tauri events per the wave brief). Each row: `entryId` + attempt-count Pill + truncated `lastError` (200 char, native `title=` tooltip surfaces the full message) + Retry button (disables mid-flight to absorb double-clicks; the backend is idempotent regardless). Empty state "No failed filings — all good!". Additive `FailedFiling` + `QueueStatus` types in `lib/types.ts`; three new IPC bindings + fixture cases in `lib/tauri.ts` (default fixtures cover the clean-queue state so `npm run preview` renders without overrides; Playwright specs override via `window.__MOCKINGBIRD_FIXTURES__` for non-empty cases). 16 new `kg.failed.*` + `kg.status.*` i18n keys (en.json). +6 IPC-contract vitest cases (4→10 in SettingsKgTab.test.ts; covers list default-empty + override + limit arg + status snapshot shape + status default-zero + status override + idempotent requeue). Tokens-only — zero hex/rgb/hsl literals, zero `any`. Gates: `npx tsc --noEmit` green, vitest 124/124 green (was 118; +6), `npm run build` green; lint skipped per `mb-yxh`. capabilities/default.json untouched (wildcard `core:default` covers all three new commands per Wave 1C.1 LESSONS). LESSONS body entry covers the file-size pre-flight discipline (initial single-file estimate hit 510 LoC — over the brief's 400-LoC extraction threshold; extracted on the second cut without over-engineering the boundary) and the `--status-bad` vs `--status-error` token drift (legacy Settings.module.css §errorBanner references `--status-bad` which doesn't exist in tokens-v2.css and falls through to a `#d33` literal; new code uses the canonical `--status-error`). Commits: this commit chain.
...
- **Failed-filings (`state='failed'` in `kg_filing_queue`)** ✅ Wave 1C.2 surfaced via `SettingsKgFailedFilings.tsx` with status line + per-row Retry (`mb-9ufg` closed). Idempotent retry contract (`kg_requeue_failed`) is now load-bearing for the Wave 1C.5 J3 invariant.
