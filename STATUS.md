...
- **Status:** Accepted 2026-05-27 (`docs/adr/0046-mobile-extension-via-vault.md`). **Iteration 1 SEALED** (desktop file-ingest pipeline). **Iteration 2 SEALED 2026-05-27** (outbound Obsidian projection live end-to-end, desktop + iPhone verified). **Iteration 3 SEALED 2026-05-27** (inbound `<vault>/inbox/` → headless ingest pipeline; iOS Shortcut hands-on install deferred to user-side setup). **Iteration 4 IN-FLIGHT** (Phases A + B + C shipped 2026-05-27/28; Phase D epic-seal pending). 3/4 iterations sealed.
- **Iteration 1: SEALED 2026-05-27.** Headless dictation ingest (`dictation::ingest::headless_ingest`) extracted from `complete()`; symphonia audio decode; `SessionsEventBus` trait; ADR §3.2 amendment (sibling `crossbeam-channel` for `HeadlessIngestRequest` alongside `StateAction`); `+ Audio file` desktop import button; migration 018 (`sessions.source`); sealed-phases-untouched judge 🟢 PASS @ 95% conf; live-fire smoke green on `New Recording 38.m4a`. Beads `mb-jqhw` / `mb-hxm4` / `mb-evn3` / `mb-7vyz` / `mb-thmd` / `mb-jbf7` all CLOSED. Seal commit `9ec2d91`.
- **Iteration 2: SEALED 2026-05-27.** Outbound projection shipping end-to-end. Beads `mb-yheh` (vault layout + manifest), `mb-p7cp` (markdown projection v1 with hand-rolled YAML + SHA-256 content addressing + golden-snapshot pin), `mb-lvzw` (8 SettingKey variants + `VaultRuntime` reconciliation engine with coalescing + post-commit triggers in dictation/meeting + 4 new `vault_*` IPCs) all CLOSED. `mb-vg3p` PARTIAL — Mobile Sync (preview) section live in Settings → Advanced; Iter 4 still owns the full dedicated tab + remaining 4 settings keys + health card. Commits: `bc077fc` / `3e8b536` / `47b35ef` / `730b6fc` / `62d12a1` / `dd48904` (impl) + `9fe79bb` / `5813d7f` (bookkeeping). Live-fire smoke green on Dustin's hands-on 2026-05-27: backfill of 90 records succeeded against `C:\Users\dboyd\mockingbird-vault\`, auto-trigger on new PTT dictation fires within seconds, iPhone (Obsidian Mobile) receives via Obsidian Sync within ~30s. Zero implementation bugs surfaced. **One operational gotcha surfaced (NOT a Mockingbird bug):** the Obsidian nested-vault trap — vault was originally created at `<root>/mockingbird-vault/mockingbird-vault/` instead of `<root>/mockingbird-vault/`, causing Mockingbird to write to the outer path while Obsidian read from the inner. Resolved via in-place migration (move `.obsidian/` + `Welcome.md` from inner to outer, delete empty inner; preserved Obsidian Sync pairing). Iter 4 setup wizard should detect + guide around this — filed as `mb-3xww` (P3, depends on `mb-vg3p`). Full diagnostic walk-through in LESSONS 2026-05-27.
- **Iteration 3: SEALED 2026-05-27.** Inbound `<vault>/inbox/` → headless ingest pipeline shipped end-to-end. **Wave 0 spike CLOSED:** 5 observation rounds run by Dustin 2026-05-24 (rounds 1 small text, 2 medium markdown, 3 large text, 4 binary attachment via Obsidian, **4b voice-memo via Files app into `inbox/`** — the spontaneously-added round that mattered most). 8 findings captured at [`docs/spikes/iter3-sync-layer-findings.md`](docs/spikes/iter3-sync-layer-findings.md), all grounded in JSONL log timestamps. Logs at `docs/spikes/iter3-logs/round-{1,2,3,4,4b}*.log`. **CRITICAL Finding 5:** external Files-app writes into Obsidian's iOS sandbox have a **5–15 minute lag** before Obsidian Mobile notices them, unless the app is foregrounded — without mitigation the Shortcut courier would have multi-minute end-to-end latency. **Mitigation shipped:** `docs/mobile/ios-shortcut.md` amended to require a third Shortcut action (`Open App: Obsidian`) as the final step; expected latency drops from minutes to ~30–60s. `mb-s8s2` closed; `mb-mus5` remains closed (amendment is a follow-up doc edit, not re-execution). **Wave 3 implementation beads chartered with findings baked into descriptions:** `mb-9lgi` (Wave 3.1 inbox watcher) **CLOSED in `a3c5900`** ← `mb-txmy` (Wave 3.2 courier processor) **CLOSED in `6b0dd40`** ← `mb-3ivf` (Wave 3.3 InboxRuntime wiring) **CLOSED in `eea2484`** ← `mb-ksau` (Wave 3.4 sealed-phases-untouched judge — **PASS @ 99% conf**, verdict at `docs/judges/mobile-extension/sealed-phases-untouched-iter3-verdict.md`) ← `mb-1yxp` (Wave 3.5 live-fire smoke — **CLOSED with deferral** per Dustin's 2026-05-27 decision; iOS Shortcut install + hands-on latency measurement is user-side setup, not gating seal). **Live-fire validation on the implementation slate is green:** the runtime's initial-scan + watcher + courier picked up a pre-existing `New Recording 38.m4a` voice memo on first launch, ran it through whisper-rs CUDA (1079ms STT) + Ollama cleanup (9866ms) into `sessions` row `session_id=116` with `source='mobile-inbox'`, and atomically archived the source to `<vault>/inbox/_archive/2026-05-24/`. This proves the pipeline is sound; the full iOS-Shortcut → Obsidian Sync → inbox/ path (which Mockingbird itself can't reach) is deferred to Dustin's own time per the `mb-1yxp` closure note. `mb-qxrm` remains a separate Iter 4 Wave 5 hardening matrix (conflict-file injection / machine-fingerprint mismatch / retention nightly / oversized silent-skip / app-offline catch-up), distinct from `mb-1yxp`'s closed Iter-3-only happy-path scope.
- **Iteration 4: IN-FLIGHT.** Polish iteration; dispatched in pieces.
  - **Phase A (`mb-vg3p`): COMPLETE.** Full Mobile Sync tab landed as a top-level Settings tab alongside General / Models / Dictation data / Meetings / Advanced. All four remaining ADR 0046 settings keys (`VaultSyncBackend`, `SyncTierByteCap`, `KeepAudioBlobs`, `VaultDebugKeepCouriers`) round-trip through the existing `vault_settings_get` / `vault_settings_set` IPCs. Two new IPCs added for the health card: `vault_runtime_status` (manifest age + last error) and `inbox_runtime_status` (running flag + last archived ISO + failed-count). UI polls both every 5 s while the tab is mounted, cleans up on unmount. Courier honors `KeepAudioBlobs`: when off, archived files are deleted after successful ingest instead of moved to `_archive/` (default true keeps existing behavior — zero regression for current users). Old `SettingsMobilePreview` section + its test file deleted (DRY — the new tab supersedes them). 32 new vitest cases on the pure helpers (`deriveOverallStatus`, `defaultByteCapForBackend`, `parseByteCapMB`, `formatRelativeMs`, `outboundBadge`, `inboxBadge`, `VAULT_DB_KEYS` mirror-of-Rust check). **Bonus:** the same dispatch also pre-positions the Phase C (`mb-3xww`) backend pieces — `detect_nested_vault` + `suggest_sibling_vault` in `vault/layout.rs` + the `vault_check_path` and `vault_ensure_dir` IPCs — sitting dormant until the wizard UI lands. Backend gates: cargo check / clippy `--release -- -D warnings` / fmt all green; `cargo test --release --no-run --lib` link-clean (full `--no-run` blocked by running app's exe lock — environmental, not a code issue). UI gates: `tsc --noEmit` clean, vitest 95/95, `npm run build` clean.
  - **Phase B (`mb-q1xt`): COMPLETE 2026-05-28** at `2fbed65`. `ingest_progress` Tauri event channel emitted at decode / transcribe / done / failed bracket points in BOTH the `+ Audio file` IPC AND the inbox courier. React `ImportProgressOverlay` (in-shell glass pill, bottom-right) listens at the app shell; auto-dismiss on done after ~1.5s, sticky on failure with manual dismiss + error quote. `dictation/ingest.rs` (sealed per ADR §3 + §3.2) UNTOUCHED — emits live at the caller side. Cleaning stage collapsed into Transcribing (the orchestrator runs them opaquely behind the headless-ingest channel); wire-format `cleaning` constant kept for forward compat. Single shared `AppIngestProgressBus` instance lives in `lib.rs::run` and is threaded into both the IPC `State` slot and `InboxRuntime::new_with_progress`, so the React overlay sees one coherent event stream regardless of origin. 9 new Rust unit tests (3 sequencing assertions on courier + 6 in the new `dictation/ingest_progress.rs` module incl. wire-format serialization) + 6 new vitest cases on the React reducer.
  - **Phase C (`mb-3xww`): COMPLETE 2026-05-28** at `8e01ad3`. `NestedVaultWizard` consumes the backend pre-positioned in `af90758` (`vault_check_path` + `vault_ensure_dir` IPCs, `VaultPathCheck` enum). When the user picks a vault path inside an existing Obsidian vault, dialog opens with three options: primary `Use sibling location` (auto-suggests `<grandparent>/mockingbird-vault`, creates via `vault_ensure_dir`, persists as `VaultPath`), secondary `Pick different` (re-opens folder picker), link-styled `Use anyway` (persists nested path + warning toast). `handleBrowse` now goes through `tryPersistVaultPath` which consults `vault_check_path` before any write. Auto-migration of vault content explicitly out of scope per kickoff — recommended branch creates an empty sibling vault; users with substantial existing content can manually copy. 6 vitest cases on the pure `siblingBranchAvailable` helper + the prop contract; full visual verification deferred to Iter 4 Playwright sweep.
  - **Phase D: READY** — epic seal (STATUS final sweep + ADR 0046 Accepted-final + LESSONS amendment if anything surfaced).  No code changes expected.
  - **No new `phase-*-complete` tag** (lateral epic per LESSONS PINNED P5).
...
- **Follow-up beads carried across iterations:**
  - `mb-0uqb` (P3) — revisit descoped sidecar-based silent-skip detection if POC users hit silent-skip in practice (descoped from §8).
  - `mb-q1xt` (P2, Iter 4 polish) — progress indicator during `+ Audio file` import (~10-60s blind wait; reuse RecordingWindow overlay with import-aware labels).
  - `mb-3xww` (P3, Iter 4 polish) — nested-vault detection + guided migration in the Mobile Sync setup flow. Linked as follow-up to `mb-vg3p`. New 2026-05-27.
...
**Discovered + filed during Iter 3 implementation slate:**
- Repo-root cleanup chore (no bead) — stale `mockingbird-mobile-extension-plan.md` (Dustin's original 2026-05-23 feature kickoff, fully superseded by ADR 0046 + this STATUS) moved to `docs/historical/2026-05-23-mobile-extension-original-kickoff.md` with a preamble noting the supersession. Commit `d010bdf`.
- The Phase A inbox file-watcher chose `src-tauri/src/inbox/` as a new top-level subsystem rather than nesting under `vault/`, matching the dictation/meetings split convention. Inbound and outbound vault flows are conceptually independent (only the path is shared); keeping them separate keeps the export-job's `VaultRuntime` and the inbox's `InboxRuntime` as parallel peers driven by the same settings gate.
