...
- **Iteration 4: READY (next dispatch).** Lifts the preview section into a full Mobile Sync tab, wires the remaining 4 settings keys (`VaultSyncBackend` / `SyncTierByteCap` / `VaultDebugKeepCouriers` / `KeepAudioBlobs`), connection health card, nested-vault wizard (`mb-3xww`), `+ Audio file` progress indicator (`mb-q1xt`), final invariant judges, ADR Accepted (final), STATUS update. Bead inventory: `mb-vg3p` (full Mobile Sync tab) + `mb-q1xt` (import progress indicator) + `mb-3xww` (nested-vault wizard). **No new `phase-*-complete` tag** (lateral epic per LESSONS PINNED P5).
