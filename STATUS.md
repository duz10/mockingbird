...
**KG Phase 1D — source-gated filing + first-class KG screen** (ADR 0052 Proposed 2026-06-04). Re-scopes the standing "Phase 1D = backfill" framing (now moot per the 2026-06-04 source-of-truth alignment review against the original product spec §15 + Clark article). Six waves; Wave 1D.0 (charter + clean-slate verify + phase doc + bead epic) **complete in this iteration**.
...
- **Clean-slate verify (Wave 1D.0):** NOT clean — `kg_entities=3`, `kg_filing_queue=2` (both `done`), `kg_entity_mentions=3`, `kg_tag_mentions=11`, `kg_graph_enabled='true'` against `sessions.id` 128 + 129 (2026-05-30 dev test toggles). Wave 1D.1 migration 025 purges all four KG mention/queue/entity tables + resets the toggle in the same transaction as the schema change. Volume is trivial; Wave 1D.6 judge asserts post-migration cleanliness.
