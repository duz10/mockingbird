...
- **Status:** Accepted 2026-05-27 (`docs/adr/0046-mobile-extension-via-vault.md`). **Iteration 1 COMPLETE** (desktop file-ingest pipeline shipped, no Obsidian touchpoints yet). **Iteration 2 IN FLIGHT** (outbound Obsidian projection: phases A+B sealed, C.1 sealed, C.2/C.3/D pending — see resume anchor below).
...
- **Iteration 2: IN FLIGHT 2026-05-27.** Three commits landed in this session under bead `mb-lvzw` partial:
  ...
  - `47b35ef` — Phase C.1 (partial `mb-lvzw`). 8 new `SettingKey` variants per ADR §10: 4 behavioral (`MobileSyncEnabled`, `VaultPath`, `VaultSyncRecordTypes`, `VaultRetentionDays`) + 4 Iter 4 stubs (`VaultSyncBackend`, `SyncTierByteCap`, `VaultDebugKeepCouriers`, `KeepAudioBlobs`). All opt-in / OFF by default per ADR §10. `all_enumerates_every_variant` bumped 28→36; new `mobile_sync_defaults_match_adr_0046` test.
  - **Resume anchor for next session — Phase C.2 + C.3 + D still TODO:**
    - **Phase C.2** (engine, no sealed-code wiring): create `src-tauri/src/vault/export_job.rs` with `VaultRuntime { config: Arc<RwLock<VaultConfig>>, job_lock: Arc<Mutex<()>>, coalesced: Arc<AtomicBool> }` + `run_once(&db)` reconciliation pass (layout.ensure_zones → manifest.load → query in-scope records → project + write_atomic if sha differs → archive stale → manifest.save_atomic) + `trigger(db)` spawning thread with coalescing-rerun semantics + `run_once_blocking(db)` for the manual IPC. Query helpers: `sessions WHERE started_at >= cutoff AND status='complete'` (filter by source) + `meeting_sessions WHERE started_at >= cutoff AND status='complete'`. Body assembly: for dictation, use `final` transcript stage with `cleaned` fallback (check `src-tauri/src/dictation/transcripts.rs`); for meeting, use `formatted_merged` (or fallback to `mic` or `sys`) from `meeting_sessions` row.
    - **Phase C.3** (sealed-code wiring): add `vault: Arc<VaultRuntime>` field to `DictationOrchestrator` (constructor at `dictation.rs:300-320`) + call `self.vault.trigger(db.clone())` at end of `persist_complete` (line 899 area) + same in `persist_failed_*`. Add `vault: Arc<VaultRuntime>` to `IngestDeps<'a>` at `dictation/ingest.rs:161` + trigger after the persist commit in `headless_ingest`. Find the struct that owns `meetings::lifecycle::finalize_meeting` (probably in `meetings/lifecycle.rs` — grep for `impl` block; likely `MeetingCaptureRuntime` per `MeetingHotkeyPaused` doc; cross-check) + add `vault: Arc<VaultRuntime>` field + trigger after `persist_meeting`. Register `VaultRuntime` in `lib.rs` Tauri builder state. Add `vault_export_now` IPC command. Trigger initial-backfill from settings-update IPC when `MobileSyncEnabled` flips false→true or `VaultPath` changes (check `commands/settings.rs` or wherever the settings IPC lives).
    - **Phase D / `mb-vg3p` partial:** new "Mobile Sync (preview)" section in `ui/src/pages/Settings.tsx` with master toggle, vault path text input + Browse button, status line, "Export now" button. Don't close `mb-vg3p` — Iter 4 owns the rest.
  - **Pending live-fire smoke (Dustin):** after C+D land, the §15 click-by-click sequence in the kickoff doc runs end-to-end (toggle ON → backfill ~112 records → confirm in Obsidian desktop → wait for Sync → confirm on iPhone → fresh PTT dictation → confirm new .md appears + syncs).
